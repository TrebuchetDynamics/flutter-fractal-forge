...
>
> **Source anchors for this refresh:** `test/catalog/catalog_id_integrity_test.dart` (501 escape-time entries, 966 production fractals, 1 scientific visualization, and 974 debug/test registry modules including 7 diagnostics), `lib/features/viewer/fractal_viewer_screen.dart` (controls HUD), `lib/core/services/rendering/palette_service.dart` + `palette_shader_adapter.dart` (palette textures).
...
- Live registry lock: 501 escape-time catalog entries, 28 raymarched-3D entries, 9 custom hand-built modules, 966 production fractals plus 1 scientific visualization (974 debug/test `ModuleRegistry` modules including 7 diagnostics)
...
**Most of the 966-production-fractal catalog is still NOT suitable for perturbation; exact category counts need a fresh catalog audit.**

### 📊 Module Registry Breakdown (966 production fractals; 974 debug/test modules)
...
├── Production fractals: 966
├── Scientific visualizations: 1
└── Debug/test ModuleRegistry modules including diagnostics: 974
...
- [x] Registry covers 501 escape-time entries + 28 raymarched 3D + shared/custom promotions = 966 production fractals plus 1 scientific visualization (974 debug/test modules including diagnostics)
...
- [x] **Target met:** Catalog grew beyond the old 370 baseline to 966 production fractals.
