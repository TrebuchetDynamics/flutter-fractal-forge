...
## Purpose
Static assets bundled with the application. Contains app icon and store image assets. Catalog thumbnails are rendered at runtime, not bundled here.
...
- Catalog thumbnails are runtime-rendered by the catalog UI; do not add `assets/catalog_thumbs/` back unless reverting that decision.
