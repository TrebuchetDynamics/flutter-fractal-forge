...
| `image` | 4.7.2 | 4.9.1 | Medium: export encoding and wallpaper transforms rely on image codecs. | Upgrade separately with `test/export/export_service_test.dart`, batch/video export tests, and sample encoded output checks. |
...
flutter test \
  ...
  test/export/video_export_service_test.dart \
  test/export/batch_export_service_test.dart
```
...
For `image`, inspect at least one PNG and one JPG encoded result via `test/export/export_service_test.dart`; add a regression fixture before changing codec behavior.
