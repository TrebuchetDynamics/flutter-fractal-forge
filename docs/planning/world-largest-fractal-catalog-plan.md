...
- Shared-renderer duplicate-risk audit: `research/worlds-largest-fractal-catalog/shared-renderer-duplicate-risk-audit.md` / `.json` is historical input. The 2026-07-29 3D audit removed fixed Mandelbox scales, Mandelbulb powers, Quaternion constants, and direct renderer aliases because parameters and presets do not create counted identities.
...
- Semantic-supported implementation batches: `research/worlds-largest-fractal-catalog/semantic-supported-implementation-batches.md` / `.json`; 3D parameter-only candidates remain non-counting and must be represented by controls or presets on their canonical module.
- Live registry gap report: `research/worlds-largest-fractal-catalog/live-registry-gap-summary.md`
- Live registry promoted ledger: `research/worlds-largest-fractal-catalog/curated-entry-ledger.live-registry.json` (generated research artifact). The live app has 966 production fractals plus 1 scientific visualization; the debug/test `ModuleRegistry` has 974 modules including 7 diagnostics.
