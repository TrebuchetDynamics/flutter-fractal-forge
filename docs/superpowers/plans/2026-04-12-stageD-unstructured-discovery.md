...
| `scripts/research/crawl/bridges_mathart.py` | Bridges Math Art conference archive (archive.bridgesmathart.org) | ~600 | 25 years × ~150 papers/year ≈ 3,500 PDFs; title+abstract LLM classifier filters to fractal/tiling/origami papers; GROBID + LLM extraction |
...
| `scripts/research/extract/bridges_parser.py` | PDF + LLM extraction; emphasizes physical/tiling/architectural fractal constructions (origami, sculpture, Islamic patterns, fractal architecture) that EN programming sources under-cover |
...
## Task 7a: bridges_mathart.py — Bridges Math Art conference archive

The Bridges conference (annual since 1998) publishes open-access proceedings at archive.bridgesmathart.org covering the intersection of mathematics and art. Strong coverage of fractal art that EN programming databases miss: origami fractals, kirigami, sacred geometry, Islamic geometric patterns, fractal architecture, tiling art, fractal sculpture, sonification.

Target categories filled: **XIV Physical & Constructed (origami, kirigami, sculpture), XVII Architectural & Structural (Islamic, sacred geometry, fractal architecture), X Tiling & Aperiodic, IV IFS & Geometric Construction, XVI Musical & Rhythmic**.

- [ ] **Step 1: Discover archive structure.** `archive.bridgesmathart.org/{year}/bridges{year}-NNN.html` lists each year's papers; each page links a `pdf-bridges{year}-NNN.pdf`. Scrape year index pages, extract paper metadata (title, authors, abstract where available).

- [ ] **Step 2: Title+abstract filter.** Not all ~3,500 papers are fractal-relevant. Run a cheap LLM classifier on title+abstract to tag each paper as `fractal_relevant: bool` with confidence. Expect ~15-20% hit rate → ~500-700 candidate papers.

- [ ] **Step 3: PDF download for relevant papers only.** Rate-limit 0.5 req/sec. Respect archive terms.

- [ ] **Step 4: GROBID + LLM extract with Bridges-specific prompt.** The prompt must handle papers that describe *physical* constructions (e.g., "fold paper into fractal pattern using these 12 steps") and emit a candidate that captures the construction sequence as parameters rather than a closed-form formula. `formula_ast.iteration_type` may be `ifs`, `l_system`, `tiling`, `other` depending on paper.

- [ ] **Step 5: 10 fixture papers spanning origami / Islamic / sculpture / tiling / sonic.** Test parser produces schema-valid candidates.

- [ ] **Step 6: Smoke run: 100 paper classification → ~15 candidates expected.**

- [ ] **Step 7: Commit.**

---

## Task 7b: cnki.py — Chinese-origin fractals (EN output)
...
- [ ] 5 crawlers operational with fixture tests (arxiv, github_glsl, fractal_forums, bridges_mathart, cnki)
- [ ] LLM extraction cached + budget-controlled
- [ ] GROBID integration tested
- [ ] 300+ new candidates admitted through Stage B (Bridges adds ~100-200 to prior ~200 target)
- [ ] Bridges archive yields ≥10 admitted fractals in categories XIV/XVII/X (physical/architectural/tiling) — the coverage-gap categories
...
| Bridges PDFs describe non-computable physical constructions | `formula_ast: null` is acceptable for these — admit as reference tier with full Dart metadata but no shader; describe construction in `candidate.description_en` and `references`. Later hand-authoring can produce shaders/interactive recreations for promotable ones. |
