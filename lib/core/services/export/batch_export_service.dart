...
class BatchExportService {
  ...
  const BatchExportService({
    ...
    ExportWorker worker = const IsolateExportWorker(),
  ...
  void cancelActiveExport() => _coordinator.cancelActive();
  ...
  Future<BatchExportResult> exportPresets({
    ...
    return _coordinator.run(ExportKind.batch, (token) async {
      if (kIsWeb) {
        throw UnsupportedError('exportPresets is not supported on web');
      }
        ...
        if (isCancelled() || token.isCancelled) {
          ...
          try {
            ...
            for (final result in results) {
              ...
              images.add(await result.file.readAsBytes());
            }
            final pngBytes = await _worker.runWithInput(
              _encodeContactSheet,
              <String, Object>{
                'images': images,
                'columns': 4,
            ...
            AppLogger.instance.warn('export', 'Failed to build contact sheet',
                data: {'error': '$e'});
          }
        }
      }
  ...
  Future<Uint8List> build(
    ...
    for (final file in files) {
      try {
        ...
        AppLogger.instance.warn('export', 'Skipping contact-sheet image',
            data: {'error': '$e'});
      }
    }
    return _encodeContactSheet(<String, Object>{
      'images': images,
      'columns': columns,
      'tileSize': tileSize,
      'padding': padding,
...
Uint8List _encodeContactSheet(Map<String, Object> input) {
  final files = (input['images']! as List).cast<Uint8List>();
  final columns = input['columns']! as int;
  final tileSize = input['tileSize']! as int;
  final padding = input['padding']! as int;
