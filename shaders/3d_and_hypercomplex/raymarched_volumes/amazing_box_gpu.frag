...
// Amazing Box — documented 3D ray-marched fractal expansion.
...
float fractalDE(vec3 p) {
    vec3 z = p;
    float scale = clamp(uPower, 1.6, 3.2);
    float dr = 1.0;
    int maxIter = int(clamp(uIterations, 4.0, 28.0));
    for (int i = 0; i < 28; i++) {
        if (i >= maxIter) break;
        z = clamp(z, -1.0, 1.0) * 2.0 - z;
        float r2 = dot(z, z);
        float k = clamp(1.0 / max(r2, 0.22), 1.0, 4.5);
        z *= k;
        dr *= k;
        z = z * scale + p;
        dr = dr * abs(scale) + 1.0;
