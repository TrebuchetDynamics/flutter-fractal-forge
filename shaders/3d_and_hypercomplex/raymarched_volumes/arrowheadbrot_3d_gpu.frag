...
// Classical quadratic Arrowheadbrot T(1,i1,j1), where j1=i1*i2, a principal
// 3D slice of the tricomplex Mandelbrot set. For c=x+y*i1+z*j1, idempotent
// decomposition gives the complex Mandelbrot parameters (x-z)+y*i and
// (x+z)+y*i.
uniform float uTime;
...
float arrowheadbrotDistance(vec3 point) {
    vec2 cMinus = vec2(point.x - point.z, point.y);
    vec2 cPlus = vec2(point.x + point.z, point.y);
    float minusDistance = complexMandelbrotDistance(cMinus);
    float plusDistance = complexMandelbrotDistance(cPlus);
    return sqrt(0.5 * (minusDistance * minusDistance +
        plusDistance * plusDistance));
}

float sceneDistance(vec3 point) {
    return arrowheadbrotDistance(point);
}
