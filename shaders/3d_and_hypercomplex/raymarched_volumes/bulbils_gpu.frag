...
// Bulbils — documented 3D ray-marched fractal expansion.
...
float fractalDE(vec3 p) {
    ...
    float dr = 1.0;
    float r = 0.0;
    float power = clamp(uPower, 3.0, 10.0);
    int maxIter = int(clamp(uIterations, 4.0, 36.0));
    for (int i = 0; i < 36; i++) {
        if (i >= maxIter) break;
        r = length(z);
        if (r > max(uBailout, 2.0)) break;
        float theta = acos(clamp(z.z / max(r, 0.0001), -1.0, 1.0));
        float phi = atan(z.y, z.x);
        dr = pow(max(r, 0.0001), power - 1.0) * power * dr + 1.0;
        float zr = pow(r, power);
        theta = theta * power + 0.18 * sin(float(i) + uTime * 0.25);
        phi = phi * power;
        z = zr * vec3(sin(theta) * cos(phi), sin(theta) * sin(phi), cos(theta)) + p;
        z += 0.045 * sin(z.yzx * 5.0 + float(i));
    }
