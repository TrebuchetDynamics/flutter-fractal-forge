...
// Classical quadratic Mousebrot T(i1,i2,j1), where j1=i1*i2, a principal 3D
// slice of the tricomplex Mandelbrot set. For c=x*i1+y*i2+z*j1, idempotent
// decomposition gives the complex Mandelbrot parameters z+(x-y)*i and
// -z+(x+y)*i.
uniform float uTime;
...
float mousebrotDistance(vec3 point) {
    vec2 cMinus = vec2(point.z, point.x - point.y);
    vec2 cPlus = vec2(-point.z, point.x + point.y);
...
float sceneDistance(vec3 point) {
    return mousebrotDistance(point);
}
