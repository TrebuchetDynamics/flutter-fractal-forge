...
vec4 getJuliaC() {
    ...
    return mag * vec4(-0.1, 0.3, -0.4, 0.4);
}

// Quaternion Julia distance estimator.
// Maps pos -> quaternion (pos.x, pos.y, pos.z, 0), iterates z = z*z + c.
float quatJuliaDE(vec3 pos) {
...
vec3 getNormal(vec3 pos) {
    float eps = 0.001;
    float d = quatJuliaDE(pos);
    return normalize(vec3(
        quatJuliaDE(pos + vec3(eps, 0.0, 0.0)) - d,
        quatJuliaDE(pos + vec3(0.0, eps, 0.0)) - d,
        quatJuliaDE(pos + vec3(0.0, 0.0, eps)) - d
    ));
}
