...
// Classical quadratic Tetrabrot T^2(1,i1,i2), a principal 3D slice of the
// tricomplex Mandelbrot set. For c=x+y*i1+z*i2, idempotent decomposition gives
// the two complex Mandelbrot parameters x+(y-z)i and x+(y+z)i.
uniform float uTime;
...
float tetrabrotDistance(vec3 point) {
    vec2 cMinus = vec2(point.x, point.y - point.z);
    vec2 cPlus = vec2(point.x, point.y + point.z);
...
float sceneDistance(vec3 point) {
    return tetrabrotDistance(point);
}
