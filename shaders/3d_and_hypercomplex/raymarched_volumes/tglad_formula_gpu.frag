...
// Tglad Formula — documented 3D ray-marched fractal expansion.
...
float fractalDE(vec3 p) {
    vec3 z = p;
    float scale = clamp(uPower, 1.6, 3.4);
    float dr = 1.0;
    int maxIter = int(clamp(uIterations, 5.0, 34.0));
    for (int i = 0; i < 34; i++) {
        if (i >= maxIter) break;
        z = abs(z + vec3(0.08, -0.04, 0.02)) - vec3(0.72, 0.58, 0.66);
        if (z.x < z.y) z.xy = z.yx;
        if (z.x < z.z) z.xz = z.zx;
        float r2 = dot(z, z);
        float inv = clamp(1.2 / max(r2, 0.20), 0.75, 3.2);
        z *= inv;
        z = z * scale + p * 0.42;
        dr = dr * abs(scale * inv) + 1.0;
    }
    float cell = boxDE(z, vec3(0.55, 0.38, 0.30));
    float core = sphereDE(z, 0.62);
    return min(abs(core), cell) / max(dr, 0.0001);
}
