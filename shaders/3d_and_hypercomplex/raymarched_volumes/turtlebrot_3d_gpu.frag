...
// Classical quadratic Turtlebrot T(i1,i2,j2), where j2=i1*i3, a principal 3D
// slice of the tricomplex Mandelbrot set. It is the intersection of a
// Mousebrot-related slice and its z-reflection, giving four complex Mandelbrot
// parameters with real parts +/-z and imaginary parts x+/-y.
uniform float uTime;
...
float turtlebrotDistance(vec3 point) {
    vec2 c0 = vec2(-point.z, point.x - point.y);
    vec2 c1 = vec2(point.z, point.x + point.y);
    vec2 c2 = vec2(point.z, point.x - point.y);
    vec2 c3 = vec2(-point.z, point.x + point.y);
...
float sceneDistance(vec3 point) {
    return turtlebrotDistance(point);
}
