...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.1 * sin(0.001 * uTime) + 0.12 * (p.x - p.y);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = (z - b) * x - d * y;
    float dy = d * x + (z - b) * y;
    float dz = c + a * z - (z2 * z) / 3.0 - (x2 + y2) * (1.0 + e * z) + f * z * x2 * x;
    ...
    float px = x + 0.2 * z;
    float py = y - 0.15 * z;
    float r2 = px * px + py * py + 0.08 * z * z;
    density += exp(-0.20 * (px * px + py * py)) + 0.16 * exp(-0.6 * abs(z));
    if (r2 > bailoutSq) { it = i + 1; break; }
  }

  if (it >= target) {
    float t = fract((density / float(target)) * 1.85 + 0.08 * atan(y, x) + uTime * 0.00005);
