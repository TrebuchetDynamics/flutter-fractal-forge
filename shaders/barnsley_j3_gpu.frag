...
// Barnsley Julia III: piecewise z=(z±1)·cfix where cfix=(0.7,0.9),
// condition on Im(z)≥0. Julia-style: z₀=pixel.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Condition on Im(z)
    vec2 shift = (z.y >= 0.0) ? (z - vec2(1.0, 0.0)) : (z + vec2(1.0, 0.0));
    // d(shift·cfix)/dz₀ = cfix·der
    der = cmul(cfix, der);
    z = cmul(shift, cfix);
