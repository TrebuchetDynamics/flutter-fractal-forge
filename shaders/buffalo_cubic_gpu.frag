...
// Buffalo Cubic: z_{n+1} = |Re(z^3)| + i|Im(z^3)| + c
// Extends Buffalo to power 3 — abs applied to BOTH Re and Im of z^3.
// The 3-fold symmetry produces richer branching than the quadratic Buffalo.
...
void main() {
  ...
  // d(|Re(z^3)|)/dc = sign(Re(z^3)) * Re(3*z^2 * der)
  // d(|Im(z^3)|)/dc = sign(Im(z^3)) * Im(3*z^2 * der)
  vec2 der = vec2(0.0);
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Derivative: d(z^3)/dc = 3*z^2*der; abs gate on BOTH components
    float z2r = x2 - y2;                                   // Re(z^2)
    float z2i = 2.0*z.x*z.y;                               // Im(z^2)
    float dx3 = 3.0*(z2r*der.x - z2i*der.y);              // Re(3z^2·der)
    float dy3 = 3.0*(z2r*der.y + z2i*der.x);              // Im(3z^2·der)
    der = vec2(sign(x3) * dx3 + 1.0, sign(y3) * dy3);

    z = vec2(abs(x3) + c.x, abs(y3) + c.y);
