...
// Buffalo⁶ Julia: z_{n+1} = |Re(z⁶)|+i|Im(z⁶)| + c, c = (−0.50, 0.50).
// Julia set of the degree-6 Buffalo map — pixel = z₀, c fixed.
// The Buffalo fold applies absolute value to both components of z⁶ after the
// power. This post-power bilateral fold creates six-armed structures where
// each arm features buffalo-horn curvature and deeply nested spiral tangles.
...
void main() {
  ...
  vec2 c   = vec2(-0.50, 0.50);  // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z5 = cmul(z4, z);
    vec2 z6 = cmul(z4, z2);
    // Derivative: 6·z⁵·der (Julia: no +1; fold approximated as identity)
    der = 6.0 * cmul(z5, der);
