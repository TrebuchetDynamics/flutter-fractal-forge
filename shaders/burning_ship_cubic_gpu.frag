...
// Extends the Burning Ship to power 3: abs applied before cubing produces
// 4-fold symmetric structure with deeper, more intricate ship-like forms.
...
void main() {
  ...
  // d(w^3)/dc = 3*w^2 * dw/dc  [complex multiply, since w treated as complex]
  vec2 der = vec2(0.0);
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Cubic Burning Ship: z = (|z.x| + i*|z.y|)^3 + c
    vec2 sz = sign(z);
    vec2 w  = abs(z);
    float wx2 = w.x*w.x;
    float wy2 = w.y*w.y;

    // Derivative: dw/dc = (sz.x*der.x, sz.y*der.y), then d(w^3)/dc = 3*w^2*dw
    vec2 dw = vec2(sz.x * der.x, sz.y * der.y);
    vec2 w2 = vec2(wx2 - wy2, 2.0*w.x*w.y);          // w^2 as complex
    der = 3.0 * vec2(w2.x*dw.x - w2.y*dw.y,
                     w2.x*dw.y + w2.y*dw.x) + vec2(1.0, 0.0);

    z = vec2(w.x*(wx2 - 3.0*wy2) + c.x, w.y*(3.0*wx2 - wy2) + c.y);
