...
// Burning Ship cubic Julia: z_{n+1} = (|Re(z)| + i|Im(z)|)^3 + c.
// Julia set of the cubic Burning Ship map — pixel = z₀, c = (−1.08, 0.22).
// The cubic absolute-value fold (applied before cubing) produces three-arm
// ship hulls with flame tendrils that are mirror-symmetric about the
// Re-axis (due to the even power of |Im|). The cubic exponent adds an
// extra twist to the spiral arms compared to the degree-2 Burning Ship Julia.
...
void main() {
  ...
  vec2 c   = vec2(-1.08, 0.22);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Reflect into first quadrant, then cube
    vec2 za = vec2(abs(z.x), abs(z.y));
    vec2 za2 = cmul(za, za);
    vec2 za3 = cmul(za2, za);
    // Derivative: 3za²·der (abs treated as ±1 for normal map)
    der = 3.0 * cmul(za2, der);
    z = za3 + c;
