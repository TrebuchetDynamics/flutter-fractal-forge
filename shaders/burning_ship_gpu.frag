...
// Matches burning_ship_module.dart indices
...
vec3 palette(float t, int scheme) {
  // 0..3 keep legacy named palettes for continuity.
  if (scheme == 0) {
    return vec3(
  ...
  // 4..63: procedurally generated palettes from the scheme index.
  float s = float(scheme);

  // Classic IQ palette form: a + b*cos(2π*(c*t + d))
...
void main() {
  ...
  // flip Y for typical Burning Ship orientation
