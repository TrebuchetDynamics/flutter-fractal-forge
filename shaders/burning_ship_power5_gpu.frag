...
// Burning Ship degree 5: z_{n+1} = (|Re(z)| + i|Im(z)|)^5 + c.
// Extends the classic Burning Ship (degree 2) to degree 5 by folding
// both components through absolute value before raising to the 5th power.
// Degree 5 produces a 4-fold petal structure on the main body with
// longer spiral arms than degree 4, and the five-fold internal symmetry
// of z^5 generates extra sail-like appendages on each hull fragment.
// Derivative: 5·(|Re|+i|Im|)^4·der + 1 for normal-map shading.
