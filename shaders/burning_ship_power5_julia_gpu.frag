...
// Burning Ship⁵ Julia set: z_{n+1} = (|Re(z)| + i|Im(z)|)^5 + c, c = (0.38, 0.08).
// Julia set of the degree-5 Burning Ship map — pixel = z₀, c = (0.38, 0.08) is fixed.
// The absolute-value fold before the degree-5 iterate creates five-armed flame structures
// with each arm exhibiting the characteristic cusped, ship-bow boundary of the Burning
// Ship family. Near c=(0.38, 0.08) the set is connected with visible arm symmetry.
// Derivative approximation: 5·(|Re|+i|Im|)⁴·der (ignores sign flip at folds).
...
void main() {
  ...
  vec2 c   = vec2(0.38, 0.08);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Derivative approximation: 5·za⁴·der
    der = 5.0 * cmul(za4, der);
    z = cmul(za4, za) + c;
