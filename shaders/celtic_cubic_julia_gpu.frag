...
// Celtic Cubic Julia set: z_{n+1} = |Re(z³)| + Im(z³) + c, c = (−0.32, 0.60).
// Julia set of the degree-3 Celtic map — pixel = z₀, c = (−0.32, 0.60) is fixed.
// The Celtic map folds only the real part of z³ through absolute value, leaving
// the imaginary part unfolded. At degree 3 this creates three-armed Julia sets
// with asymmetric left-right character: the right arms retain the smooth cusp
// boundary of z³ while the folded-real axis produces doubled lobes.
// Near c=(−0.32, 0.60) the set is connected with distinct trilateral structure.
// Derivative approximation: 3z²·der (Julia mode, ignores Re fold).
...
void main() {
  ...
  vec2 c   = vec2(-0.32, 0.60);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z2 = cmul(z, z);
    vec2 z3 = cmul(z2, z);
    // Derivative approximation: 3z²·der (Julia mode: no +1)
    der = 3.0 * cmul(z2, der);
    // Celtic cubic fold: |Re(z³)| + Im(z³)
