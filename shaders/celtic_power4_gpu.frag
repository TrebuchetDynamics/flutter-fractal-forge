...
// Celtic degree 4: z_{n+1} = |Re(z^4)| + Im(z^4) + c.
// Extends the classic Celtic Mandelbrot (degree 2: |Re(z²)|+Im(z²)+c)
// to degree 4 by folding only the real part of z^4 via absolute value,
// leaving the imaginary part unmodified. The single-axis fold produces
// asymmetric structures: top-bottom mirror symmetry is broken while
// left-right symmetry from |Re| is preserved. Compared to Buffalo ⁴
// (which folds both components), Celtic ⁴ has richer vertical detail.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z3 = cmul(z2, z);
    vec2 z4 = cmul(z2, z2);
    // Derivative approximation: d[|Re(z^4)|+Im(z^4)]/dc ≈ 4z³·der + 1
    der = 4.0 * cmul(z3, der) + vec2(1.0, 0.0);
    // Celtic fold: |Re| of z^4, keep Im of z^4
