...
// Celtic⁴ Julia set: z_{n+1} = |Re(z⁴)| + Im(z⁴) + c, c = (−0.52, 0.42).
// Julia set of the degree-4 Celtic map — pixel = z₀, c = (−0.52, 0.42) is fixed.
// The Celtic map folds only the real part of z⁴ through absolute value, leaving
// the imaginary part unfolded. This asymmetric fold produces Julia sets with
// top-bottom asymmetry and interlocking lobe structures reminiscent of Celtic
// knotwork patterns. Near c=(−0.52, 0.42) the set is connected with visible
// fourfold base symmetry broken by the one-sided fold.
// Derivative approximation: 4·z³·der (ignores abs fold on Re only).
...
void main() {
  ...
  vec2 c   = vec2(-0.52, 0.42);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z3 = cmul(z2, z);
    vec2 z4 = cmul(z2, z2);
    // Derivative approximation: 4·z³·der
    der = 4.0 * cmul(z3, der);
    // Celtic fold: |Re(z⁴)| + Im(z⁴)
    z = vec2(abs(z4.x), z4.y) + c;
