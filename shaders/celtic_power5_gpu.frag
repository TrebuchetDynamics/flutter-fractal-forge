...
// Celtic degree 5: z_{n+1} = |Re(z^5)| + Im(z^5) + c.
// Extends the Celtic fractal family to degree 5 by folding only the
// real part of z^5 via absolute value, leaving Im(z^5) unchanged.
// The degree-5 Celtic has 4-fold petal decorations on each bulb, and
// the one-axis fold preserves left-right symmetry while breaking
// top-bottom symmetry, creating longer-armed asymmetric spiral tendrils
// than the degree-4 Celtic version.
// Derivative approximation: 5z^4·der + 1 (abs fold on Re ignored).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Celtic fold: |Re| of z^5, keep Im of z^5
