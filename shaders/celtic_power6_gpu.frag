...
// Celtic⁶ set: z_{n+1} = |Re(z^6)| + Im(z^6) + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// The degree-6 Celtic map folds only the real part of z⁶ through absolute
// value. This one-sided fold breaks the bilateral symmetry of the pure z⁶
// Multibrot: the resulting parameter space has six-fold rotational symmetry
// but no reflection symmetry, producing Celtic knotwork-like interlocking
// boundary structures. At degree 6 the Celtic map develops highly intricate
// overlapping lobe chains with deep cusp valleys separating each pair of
// adjacent arms. Smooth coloring: log₂(6).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z5 = cmul(z4, z);
    vec2 z6 = cmul(z4, z2);
    // Derivative approximation: 6·z⁵·der + 1 (Mandelbrot mode)
    der = 6.0 * cmul(z5, der) + vec2(1.0, 0.0);
    // Celtic fold: |Re(z⁶)| + Im(z⁶)
    z = vec2(abs(z6.x), z6.y) + c;
