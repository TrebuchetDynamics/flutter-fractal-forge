...
// Celtic⁶ Julia: z_{n+1} = |Re(z⁶)|+Im(z⁶) + c, c = (−0.30, 0.50).
// Julia set of the degree-6 Celtic map — pixel = z₀, c fixed.
// The Celtic fold applies absolute value only to Re(z⁶), leaving Im(z⁶)
// unchanged. This one-sided fold breaks bilateral symmetry, producing
// Celtic knotwork-like asymmetric arm structures with interlocking lobes.
...
void main() {
  ...
  vec2 c   = vec2(-0.30, 0.50);  // fixed Julia parameter
