...
// Chen-Lee attractor (2004) — a=5, b=−10, c=−0.38.
// dx/dt = ax − yz   = 5x − yz
// dy/dt = by + xz   = −10y + xz
// dz/dt = cz + xy/3 = −0.38z + xy/3
// Discovered by Chen & Lee while studying the rigid-body dynamics of a
// gyroscope. Despite its simple quadratic structure it produces a complex
// multi-lobe attractor with sensitive dependence on initial conditions.
uniform float uTime;
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 1.5 + 0.3 * sin(0.0017 * uTime) + 0.15 * p.x;

  const float a_p =  5.0;
  const float b_p = -10.0;
  const float c_p = -0.38;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = a_p*x - y*z;
    float dy = b_p*y + x*z;
    float dz = c_p*z + x*y / 3.0;
    x += dt * dx;
    y += dt * dy;
    z += dt * dz;
    float r2 = x*x + y*y + 0.1*z*z;
    orbit += exp(-0.08 * (x*x + y*y)) + 0.05 * exp(-0.2 * abs(z));
