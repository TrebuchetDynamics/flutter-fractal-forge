...
float hChua(float x, float m0, float m1) {
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.15 * sin(0.001 * uTime) + 0.12 * (p.x - p.y);

  const float alpha = 15.6;
  const float beta = 28.0;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float r2 = x * x + y * y + 0.15 * z * z;
    density += exp(-0.18 * r2) + 0.15 * exp(-1.2 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
  ...
  if (it >= target) {
    float t = fract((density / float(target)) * 1.7 + 0.07 * atan(y, x) + uTime * 0.00005);
