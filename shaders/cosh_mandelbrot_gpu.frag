...
// Cosh Mandelbrot: z_{n+1} = cosh(z) + c
// Hyperbolic cosine iteration; symmetric about real axis. The bulb structures
// mirror those of the cosine Mandelbrot rotated by 90°.
// Derivative: d(cosh(z))/dc = sinh(z)·der + 1.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    if (j >= target) break;
    // d(cosh(z))/dc = sinh(z)·der + 1
    der = cmul(csinh(z), der) + vec2(1.0, 0.0);
    z   = ccosh(z) + c;
