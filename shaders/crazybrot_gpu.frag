...
// Crazybrot — from MandlebrotSetSFML formula #12.
// z_{n+1} = 1/z + c  (complex inversion: conj(z)/|z|²).
// Inversion maps exterior to interior and vice-versa; the Mandelbrot parameter
// space develops "balls" at the origin that are visible in the Julia sets.
// Derivative: der_{n+1} = −1/z² · der_n + 1  (Mandelbrot style).
...
void main() {
  ...
  vec2 z   = vec2(0.0);  // z₀ = 0, Mandelbrot style
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float z2 = max(1e-12, dot(z, z));
    // der_{n+1} = d(1/z)/dz · der + 1 = −1/z² · der + 1
    der = cmul(cdiv(vec2(-1.0, 0.0), cmul(z, z)), der) + vec2(1.0, 0.0);
    // 1/z = conj(z)/|z|²
    z = vec2(z.x, -z.y) / z2 + c;
