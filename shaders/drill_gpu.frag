...
// Drill Fractal — from MandlebrotSetSFML formula #8.
// z_{n+1} = √π · z² + c.
// Multiplying the standard Mandelbrot step by √π ≈ 1.7724 stretches the
// escape basins, producing elongated drill-bit-like filaments radiating
// from the central body. The extra scale factor shifts fixed points and
// changes the boundary curvature significantly.
// Derivative: der_{n+1} = √π · 2z · der + 1.
...
void main() {
  ...
  const float SQRT_PI = 1.7724538509055159;   // √π

  for (int j = 0; j < MAX_ITERS; j++) {
    if (j >= target) { it = target; break; }

    // der_{n+1} = √π · 2z · der + 1
    der = SQRT_PI * 2.0 * cmul(z, der) + vec2(1.0, 0.0);
    // z_{n+1} = √π · z² + c
    z   = SQRT_PI * cmul(z, z) + c;
