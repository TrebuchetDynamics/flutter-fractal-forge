...
// Ducky Fractal: z_{n+1} = (z² + c) / (z² − 1)  (Mandelbrot-style, z₀=0, c=pixel)
// The rational map creates fractal "duck" silhouettes near the poles z²=1,
// with intricate Möbius-transformed copies of the Mandelbrot boundary.
// Derivative (dz/dc): exact complex quotient rule, der₀=(0,0).
//   derNew = ((d(z²+c)/dc)·(z²-1) - (z²+c)·d(z²-1)/dc) / (z²-1)²
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Exact quotient rule: d(num/den)/dc = (dnum·den - num·dden) / den²
    // dnum = d(z²+c)/dc = 2z·der + 1,  dden = d(z²-1)/dc = 2z·der
    vec2 dz2dc = 2.0 * cmul(z, der);
    vec2 dnum  = dz2dc + vec2(1.0, 0.0);
    vec2 dden  = dz2dc;
    der = cdivSafe(cmul(dnum, den) - cmul(num, dden), cmul(den, den));
    z   = cdivSafe(num, den);
