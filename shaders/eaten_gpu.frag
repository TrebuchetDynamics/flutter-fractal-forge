...
// Eaten fractal — from MandlebrotSetSFML formula #16.
// a = z²;  z_{n+1} = a + c / (a + 0.1)
// The constant 0.1 offsets the denominator, preventing the true singularity at
// a=0 while creating "eaten" notches in the standard Mandelbrot shape.
// Derivative: der_{n+1} ≈ 2z·der + 1/(z²+0.1)  (Mandelbrot style).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 a = cmul(z, z);                // z²
    vec2 b = a + vec2(0.1, 0.0);        // z² + 0.1  (offset denominator)
    // der_{n+1} ≈ 2z·der + 1/(z²+0.1)
    der = 2.0 * cmul(z, der) + cdiv(vec2(1.0, 0.0), b);
    z = a + cdiv(c, b);                 // z² + c/(z²+0.1)
