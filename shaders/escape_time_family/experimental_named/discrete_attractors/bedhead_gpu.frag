...
uniform float uParamA;  // slot 10, default -0.81
uniform float uParamB;  // slot 11, default -0.92
...
void main() {
  ...
  float a = abs(uParamA) < 1e-6 ? -0.81 : uParamA;
  float b = abs(uParamB) < 1e-6 ? -0.92 : uParamB;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float nx = sin(x * y / bSafe) * y + cos(a * x - y);
    float ny = x + sin(y) / bSafe;
    x = nx;
    y = ny;
