...
// Jason Rampe 1 attractor — x' = cos(y*b)+c*sin(x*b), y' = cos(x*a)+d*sin(y*a).
...
uniform float uParamA;  // slot 10, default 1.7
uniform float uParamB;  // slot 11, default 1.7
uniform float uParamC;  // slot 12, default 0.6
uniform float uParamD;  // slot 13, default 1.2
...
void main() {
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float nx = cos(y * b) + c * sin(x * b);
    float ny = cos(x * a) + d * sin(y * a);
    x = nx;
    y = ny;
