...
// Jason Rampe 2 attractor — x' = cos(y*b)+c*cos(x*b), y' = cos(x*a)+d*cos(y*a).
...
uniform float uParamA;  // slot 10, default 2.1
uniform float uParamB;  // slot 11, default -2.3
uniform float uParamC;  // slot 12, default 0.8
uniform float uParamD;  // slot 13, default -0.7
...
void main() {
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float nx = cos(y * b) + c * cos(x * b);
    float ny = cos(x * a) + d * cos(y * a);
    x = nx;
    y = ny;
