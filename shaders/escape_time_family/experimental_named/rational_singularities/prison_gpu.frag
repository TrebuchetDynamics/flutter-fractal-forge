...
// Prison fractal — from MandlebrotSetSFML formula #15.
// w = (sin(Re z), sin(Im z));  z_{n+1} = w² + c.
// Component-wise sine then complex-square: banded structures resemble prison bars
// in Julia views. Julia-like banding visible even in Mandelbrot parameter space.
// Derivative: der_{n+1} = 2·w·(cos x + i·cos y)·der_n + 1  (Mandelbrot style).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 w  = vec2(sin(z.x), sin(z.y));  // component-wise sine
    vec2 dw = vec2(cos(z.x), cos(z.y));  // derivative: d(sin(x,y))/d(x,y)
    // der_{n+1} = 2·w · (cos(x)+i·cos(y)) · der_n + 1
    der = 2.0 * cmul(w, cmul(dw, der)) + vec2(1.0, 0.0);
    z = cmul(w, w) + c;
  ...
  if (it >= target) {
    float bars = smoothstep(0.28, 0.50, max(abs(sin(10.0 * c.x)), abs(sin(10.0 * c.y))));
    float phase = atan(c.y, c.x) / 6.28318530718 + 0.5;
    float tBound = fract(phase + 0.26 * bars + 0.08 * length(c) + uTime * 0.0001);
    vec3 col = palette(tBound, schemeInt) * (0.52 + 0.48 * bars);
