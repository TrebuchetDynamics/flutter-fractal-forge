...
void main() {
  ...
  float delay = float(it) / float(target);
  float t = 1.0 - delay; // invert indexing
  vec3 base = getPaletteColor(t, int(uColorScheme));
  float brightness = smoothstep(0.0, 1.0, delay);
  vec3 color = mix(base * 0.25, base * 1.35, brightness);
