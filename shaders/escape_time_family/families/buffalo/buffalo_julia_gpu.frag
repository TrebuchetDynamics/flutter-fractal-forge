...
void main() {
  ...
  vec2 cSeed = vec2(-0.45, 0.1428);
  // dz/dz0 derivative. Buffalo Julia: z = |Re(z^2)| + i|Im(z^2)| + cSeed.
  // sign gate on both Re(z^2) and Im(z^2).
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float dy2 = 2.0 * (z.x * der.y + z.y * der.x);
    der = vec2(sign(z2.x) * dx2, sign(z2.y) * dy2);
    z = vec2(abs(z2.x), abs(z2.y)) + cSeed;
