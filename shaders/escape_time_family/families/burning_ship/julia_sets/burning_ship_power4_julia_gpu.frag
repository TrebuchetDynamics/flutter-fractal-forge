...
// Burning Ship⁴ Julia set: z_{n+1} = (|Re(z)| + i|Im(z)|)^4 + c, c = (−0.52, 0.54).
// Julia set of the degree-4 Burning Ship map — pixel = z₀, c = (−0.52, 0.54) is fixed.
// The absolute-value fold is applied to both real and imaginary parts before raising
// to the fourth power, producing four-fold flame-like structures with crescent cusps.
// Derivative approximation: 4·(|Re|+i|Im|)³·der (ignores sign flip at folds).
...
void main() {
  ...
  vec2 c   = vec2(-0.52, 0.54);   // fixed Julia parameter
