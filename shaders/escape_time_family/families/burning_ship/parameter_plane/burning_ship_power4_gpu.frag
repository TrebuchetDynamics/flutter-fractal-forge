...
// Burning Ship degree 4: z_{n+1} = (|Re(z)| + i|Im(z)|)^4 + c.
// At each step the orbit is reflected into the first quadrant before
// raising to the 4th power, doubling the number of fold symmetries
// compared to the classic Burning Ship. Produces a distinctive
// four-pronged ship hull with mirrored flame structures.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Reflect into first quadrant, then raise to power 4
    vec2 za = vec2(abs(z.x), abs(z.y));
    vec2 z2 = cmul(za, za);
    vec2 z4 = cmul(z2, z2);
    // Derivative: d(|z|^4)/dc ≈ 4·za^3·der + 1 (treating abs as approx identity for normal map)
    vec2 z3 = cmul(z2, za);
