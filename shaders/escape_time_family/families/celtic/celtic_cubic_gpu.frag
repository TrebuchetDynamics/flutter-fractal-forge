...
// Celtic Cubic: z_{n+1} = |Re(z_n^3)| + i·Im(z_n^3) + c
// Extends Celtic to power 3 — abs applied to the real part of z^3 only.
// The 3-fold symmetry of z^3 combined with the Celtic abs-fold produces
// aperiodic structures richer than either parent fractal alone.
...
void main() {
  ...
  // d(Re(z^3))/dc = Re(3*z^2 * der)
  // d|Re(z^3)|/dc = sign(Re(z^3)) * d(Re(z^3))/dc
  vec2 der = vec2(0.0);
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Derivative: d(z^3)/dc = 3*z^2*der, abs gate on Re only
    float z2r = x2 - y2;             // Re(z^2)
    float z2i = 2.0*z.x*z.y;        // Im(z^2)
    float dx3 = 3.0*(z2r*der.x - z2i*der.y);  // d(Re(z^3))/dc
    float dy3 = 3.0*(z2r*der.y + z2i*der.x);  // d(Im(z^3))/dc
    der = vec2(sign(x3) * dx3 + 1.0, dy3);

    z = vec2(abs(x3) + c.x, y3 + c.y);
