...
// Mandelbar cubic (Tricorn degree 3): z_{n+1} = conj(z)^3 + c.
// The "Tricorn" family uses complex conjugation before the power,
// making the map anti-holomorphic. Degree-3 version produces 3-fold
// cusp-like bulbs (compared to degree-2 Tricorn's single cusp) and
// a main body with three primary petals arranged in a star. Unlike
// the degree-2 Tricorn (which has one period-2 bulb), the cubic
// version has period-2 bulbs at three symmetric locations.
// Derivative: 3·conj(z)²·conj(der) + 1 for normal-map shading.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Conjugate z, then raise to power 3
    vec2 zc   = vec2(z.x, -z.y);      // conj(z)
    vec2 derc = vec2(der.x, -der.y);  // conj(der)
    vec2 zc2 = cmul(zc, zc);
    // Derivative: 3·conj(z)²·conj(der) + 1
    der = 3.0 * cmul(zc2, derc) + vec2(1.0, 0.0);
    z = cmul(zc2, zc) + c;
