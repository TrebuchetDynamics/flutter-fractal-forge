...
// Mandelbar⁷ (Tricorn⁷): z_{n+1} = conj(z)^7 + c.
// Anti-holomorphic Mandelbrot analogue of degree 7 — pixel = c, z₀ = 0.
// The Mandelbar/Tricorn map uses conjugation: z_{n+1} = z̄^7 + c.
// Odd degree produces seven-fold rotational symmetry with no bilateral
// reflection symmetry in c-space. The seven-armed star structure features
// characteristic Tricorn cusps at every bulb boundary, arising from the
// interaction of holomorphic and anti-holomorphic dynamics.
// Derivative: d_{n+1} = 7·conj(z)^6·conj(d_n) + 1 (anti-holomorphic chain rule).
