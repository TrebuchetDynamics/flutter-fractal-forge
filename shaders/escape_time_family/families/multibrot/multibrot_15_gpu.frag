...
// Multibrot d=1.5: z_{n+1} = z^1.5 + c
// Non-integer power via polar form: z^p = |z|^p · e^(ip·arg(z)).
// At p=1.5 the filled Julia set has a single arm between the 1-bulb (p=1)
// and the standard Mandelbrot (p=2), with characteristic one-sided spirals.
// Derivative: d(z^1.5)/dc = 1.5 · z^0.5 · der + 1.
...
// Complex power z^p via polar form (p need not be integer).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z05 = cpow(z, 0.5);
    der = 1.5 * cmul(z05, der) + vec2(1.0, 0.0);
  ...
  // Smooth coloring correction for power 1.5
  float smoothVal = float(it) - log2(log2(mag2)) / log2(1.5);
