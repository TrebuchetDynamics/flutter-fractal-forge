...
// Multijulia⁹ set: z_{n+1} = z^9 + c, c = (−0.28, 0.08).
// Julia set of the degree-9 Multibrot map — pixel = z₀, c = (−0.28, 0.08) fixed.
// The degree-9 Julia set has nine-fold base symmetry, producing nine primary
// arms radiating from a central connected body. At c near the boundary of the
// degree-9 Multibrot set, the Julia set is connected with visible nonagonal
// (9-sided) symmetry. Each of the nine arms branches into sub-arms following
// the same pattern, creating a fractal tree structure with extraordinarily
// fine detail at the branch tips. Smooth coloring: log₂(9).
...
void main() {
  ...
  vec2 c   = vec2(-0.28, 0.08);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z8 = cmul(z4, z4);
    vec2 z9 = cmul(z8, z);
    // Derivative: 9·z⁸·der (Julia mode: no +1)
    der = 9.0 * cmul(z8, der);
    z = z9 + c;
