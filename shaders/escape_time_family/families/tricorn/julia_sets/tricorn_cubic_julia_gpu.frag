...
// Tricorn Cubic Julia set: z_{n+1} = conj(z)^3 + c, c = (0.10, 0.55).
// Julia set of the degree-3 Tricorn (Mandelbar) map — pixel = z₀, c = (0.10, 0.55) fixed.
// The degree-3 Tricorn conjugates z before cubing, creating an anti-holomorphic map
// with three-fold symmetry. Julia sets of odd-degree anti-holomorphic maps have
// (d−1) = 2 critical points, producing connected Julia sets with three primary
// arms bounded by distinctive needle-shaped cusps — a hallmark of anti-holomorphic
// dynamics. Near c=(0.10, 0.55) the set is connected with radially symmetric
// arms along the three cube roots of the real axis.
// Derivative: 3·conj(z)²·conj(der) — anti-holomorphic, Julia mode (no +1 term).
...
void main() {
  ...
  vec2 c   = vec2(0.10, 0.55);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Anti-holomorphic derivative: 3·conj(z)²·conj(der), Julia mode (no +1)
    der = 3.0 * cmul(zc2, derc);
    z = zc3 + c;
