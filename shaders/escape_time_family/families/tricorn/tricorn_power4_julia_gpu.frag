...
// Tricorn⁴ Julia set: z_{n+1} = conj(z)^4 + c, c = (0.12, 0.60).
// Julia set of the degree-4 Tricorn (Mandelbar) map — pixel = z₀, c = (0.12, 0.60) fixed.
// The Tricorn map conjugates z before raising to the fourth power, making it
// anti-holomorphic. The Julia set exhibits fourfold symmetry with distinctive
// needle-shaped cusps and Fatou components bounded by smooth arcs — a hallmark
// of anti-holomorphic dynamics. Near c=(0.12, 0.60) the set is connected with
// visible symmetry axes along the diagonals.
// Derivative: 4·conj(z)³·conj(der) — anti-holomorphic, Julia mode (no +1 term).
...
void main() {
  ...
  vec2 c   = vec2(0.12, 0.60);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Anti-holomorphic derivative: 4·zc³·conj(der), Julia mode (no +1)
    der = 4.0 * cmul(zc3, derc);
    z = zc4 + c;
  ...
  float smoothVal = float(it) - log2(log2(mag2)) / 2.0;  // log2(4) = 2
