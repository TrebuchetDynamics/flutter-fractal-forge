...
// Feather Julia set: z_{n+1} = z^3 / (1 + |z|^2) + c, c = (0.35, −0.05).
// Julia set of the Feather map — pixel = z₀, c = (0.35, −0.05) is fixed.
// The Feather map divides the cubic iterate by the scalar (1+|z|²),
// softening the growth rate and creating asymmetric feather-like
// filaments. Unlike polynomial Julia sets, the bounded component is
// only weakly compact — the Julia set has hair-like extensions radiating
// outward from a central connected body. Near c=(0.35, −0.05) the set
// is connected with three primary arms decorated by fine plume structures.
// Derivative: 3z²·der / (1+|z|²) for normal-map shading (approximation).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Derivative approximation: 3z²·der / (1+|z|²)
    der = 3.0 * cmul(z2, der) / denom;
    z = z3 / denom + c;
