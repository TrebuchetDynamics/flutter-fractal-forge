...
// LightningBrot — from MandlebrotSetSFML formula #14.
// z_{n+1} = z² + cos(arg z) · c.
// The constant c is modulated by cos(angle of z), causing it to fade in and out
// with the orbit direction. Creates lightning-bolt shapes in Julia views with
// the right palette (suggested: oscillating grayscale / colorScheme 3).
// Derivative approx: der_{n+1} = 2z·der + cos(arg z)  (Mandelbrot style).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // der_{n+1} = 2z·der + cos(arg z)
    der = 2.0 * cmul(z, der) + vec2(cos_a, 0.0);
    z   = cmul(z, z) + cos_a * c;
  ...
  if (it >= target) {
    ...
    float bolt = smoothstep(0.28, 0.52, abs(sin(18.0 * c.x + 5.0 * sin(10.0 * c.y + length(z)))));
    float branch = smoothstep(0.35, 0.58, abs(sin(14.0 * c.y + 8.0 * phase)));
    float tBound = fract(phase + 0.22 * bolt + 0.12 * branch + uTime * 0.0001);
    vec3 col = palette(tBound, schemeInt) * (0.45 + 0.55 * max(bolt, branch));
