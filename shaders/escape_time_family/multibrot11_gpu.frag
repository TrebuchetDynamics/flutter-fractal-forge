...
// Multibrot¹¹ set: z_{n+1} = z^11 + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// The degree-11 Multibrot set has eleven-fold rotational symmetry — an
// undecagonal arrangement of eleven primary bulbs around the central body.
// Odd degree produces no bilateral reflection symmetry, so the eleven arms
// radiate with a chiral twist. z^11 computed as z⁸·z³ = z⁸·(z²·z) via
// squarings for efficiency. Smooth coloring: log₂(11).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z10 = cmul(z8, z2);
    vec2 z11 = cmul(z8, z3);
    // Derivative: 11·z¹⁰·der + 1 (Mandelbrot mode)
    der = 11.0 * cmul(z10, der) + vec2(1.0, 0.0);
    z = z11 + c;
