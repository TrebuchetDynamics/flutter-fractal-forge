...
// Multibrot degree 7: z_{n+1} = z^7 + c.
// Generalized Mandelbrot set of the septimal power map.
// Degree-7 maps have 6-fold rotational symmetry: the main cardioid is
// surrounded by 6 primary bulbs, each decorated with 6 secondary bulbs,
// producing a recursive hexagonal snowflake structure. The critical
// point is z=0 (as for all z^n+c). At high zoom the spiral arms show
// 6-fold symmetry with fine filaments between each.
// Derivative: 7z^6·der + 1 for normal-map shading.
// Smooth coloring: it − log₂(log₂(|z|²)) / log₂(7).
