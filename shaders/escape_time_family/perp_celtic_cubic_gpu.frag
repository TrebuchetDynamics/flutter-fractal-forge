...
// Perpendicular Celtic Cubic: z_{n+1} = Re(z^3) + i|Im(z^3)| + c
// Abs applied to Im(z^3) only — the cubic counterpart to Perpendicular Celtic.
// Compare: Celtic Cubic applies abs to Re(z^3); this applies abs to Im(z^3).
// Produces 3-fold structures with distinct asymmetric fold along the real axis.
...
void main() {
  ...
  // d(Im(z^3))/dc = Im(3*z^2*der); abs gate on Im only.
  vec2 der = vec2(0.0);
