...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float scaleR = pow(r, -b);
    z = cmul(z, rot) * scaleR + c;
    if (dot(z, z) > bailoutSq) { it = j; break; }
  }

  if (it >= target) {
    float phase = atan(z.y, z.x) / 6.28318530718 + 0.5;
    float spiral = smoothstep(0.30, 0.50, abs(sin(18.0 * phase + 6.0 * log(1.0 + length(z)) + 14.0 * uv.x)));
    float tBound = fract(phase + 0.24 * spiral + 0.10 * length(z) + uTime * 0.0001);
    vec3 col = getPaletteColor(tBound, int(uColorScheme)) * (0.56 + 0.44 * spiral);
