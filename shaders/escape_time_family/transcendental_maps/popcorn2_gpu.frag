...
void main() {
  ...
  float h = 0.04 * max(0.25, uBailout);
  float a = 1.0 + 0.5 * sin(0.13 * uTime) + 0.2 * uCenter.x;
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float x = z.x;
    float y = z.y;
    float ta = clamp(tan(a * y), -50.0, 50.0);
    float tb = clamp(tan(a * x), -50.0, 50.0);
    float xn = x - h * sin(y + ta);
    float yn = y - h * sin(x + tb);
    z = vec2(xn, yn);
  ...
  if (it >= target) {
    ...
    float kernels = smoothstep(0.30, 0.50, abs(sin(12.0 * z.x + 15.0 * z.y + 18.0 * uv.x)));
    float tBound = fract(phase + 0.24 * kernels + 0.08 * length(z) + uTime * 0.0001);
    vec3 col = palette(tBound, int(uColorScheme)) * (0.56 + 0.44 * kernels);
    fragColor = vec4(linearToSRGB(col), uTransparentBg > 0.5 ? 0.9 : 1.0);
    return;
  }

  float t = fract(float(it) / max(1.0, uIterations) + 0.1 * sin(z.x + z.y) + uTime * 0.0001);
