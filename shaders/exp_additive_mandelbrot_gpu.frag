...
// Exponential Additive Mandelbrot: z_{n+1} = z * exp(-z) + c
// Combines multiplicative and exponential dynamics for rich structure.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // z * exp(-z) + c
    vec2 neg_z = vec2(-z.x, -z.y);
    vec2 expnz = cx_exp(neg_z);
    z = cx_mul(z, expnz) + c;
