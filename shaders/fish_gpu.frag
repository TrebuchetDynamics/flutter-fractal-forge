...
// Fish Fractal: z_{n+1} = (|Re(z²)|, -|Im(z²)|) + c  (Mandelbrot-style, z₀=0, c=pixel)
// Double abs-fold: reflects both components of z² into a quadrant,
// producing fish-fin shapes and bilateral symmetry across both axes.
// Derivative (dz/dc): approx 2z·der + 1, treating sign-folds as locally constant.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Fish: z = (|Re(z²)|, -|Im(z²)|) + c
    // Approx derivative: same as z² (treats abs sign-folds as locally constant)
    der = 2.0 * cmul(z, der) + vec2(1.0, 0.0);

    float x2 = z.x * z.x - z.y * z.y;
    float y2 = 2.0 * z.x * z.y;
    z = vec2(abs(x2), -abs(y2)) + c;
