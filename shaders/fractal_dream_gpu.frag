...
// Fractal Dream attractor — x' = sin(y*b)+c*sin(x*b), y' = sin(x*a)+d*sin(y*a).
// Discrete 2D map iterated from pixel position; escape-time coloring.
uniform float uTime;
...
uniform float uParamA;  // slot 10, default 1.468
uniform float uParamB;  // slot 11, default 2.407
uniform float uParamC;  // slot 12, default 0.194
uniform float uParamD;  // slot 13, default 1.438
...
void main() {
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float nx = sin(y * b) + c * sin(x * b);
    float ny = sin(x * a) + d * sin(y * a);
    x = nx;
    y = ny;
