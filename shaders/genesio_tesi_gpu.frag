...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.08 * sin(0.001 * uTime + p.x * 0.8 + p.y * 1.1);
  ...
  float bailoutSq = max(20.0, uBailout * uBailout);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    orbit += exp(-0.19 * (x * x + y * y)) + 0.10 * exp(-0.7 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
  ...
  if (it >= target) {
    float t = fract((orbit / float(target)) * 2.05 + 0.06 * atan(y, x) + uTime * 0.00005);
