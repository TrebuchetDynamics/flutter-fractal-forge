...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.1 * cos(0.001 * uTime + p.y * 1.3);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float r2 = x * x + y * y + 0.18 * z * z;
    orbit += exp(-0.18 * (x * x + y * y)) + 0.08 * exp(-0.6 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
  ...
  if (it >= target) {
    float t = fract((orbit / float(target)) * 2.0 + 0.05 * atan(y, x) + uTime * 0.00005);
