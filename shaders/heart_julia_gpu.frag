...
// Heart Julia set: z_{n+1} = (Re(z) + i|Im(z)|)^2 + c, c = (−0.60, 0.40).
// Julia set of the Heart map — pixel = z₀, c = (−0.60, 0.40) is fixed.
// The Heart map folds only the imaginary part through absolute value
// before squaring: w = (x+i|y|), z_{n+1} = w²+c. This produces Julia
// sets with the same Im-fold asymmetry as the Heart Mandelbrot but with
// fixed parameter. Near c=(−0.60, 0.40) the Julia set is connected,
// showing a flame-shaped central body with asymmetric trailing filaments
// — the Im fold causes the set to "point" in one direction.
// Derivative: 2(Re+i|Im|)·der (Julia mode, no +1 term).
...
void main() {
  ...
  vec2 c   = vec2(-0.60, 0.40);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Fold Im only: w = (Re(z), |Im(z)|)
    vec2 w = vec2(z.x, abs(z.y));
