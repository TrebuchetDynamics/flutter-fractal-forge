...
// Barnsley J1: piecewise linear  (Mandelbrot-style, z₀=0, c=pixel)
//   z_{n+1} = (z−1)·c   if Re(z) ≥ 0
//   z_{n+1} = (z+1)·c   if Re(z) < 0
// Discovered by Michael Barnsley; produces pointed, bird-wing-shaped attractors.
// Derivative (dz/dc): exact piecewise product rule, der₀=(0,0).
//   derNew = c·der + (z±1)  (branch treated as locally constant)
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // d/dc[(z±1)·c] = c·der + (z±1)
    vec2 shift = (z.x >= 0.0) ? (z - vec2(1.0, 0.0)) : (z + vec2(1.0, 0.0));
    vec2 derNew = cmul(der, c) + shift;
    z   = cmul(shift, c);
    der = derNew;
  ...
  if (it >= target) {
    ...
    float folds = smoothstep(0.34, 0.50, abs(sin(11.0 * z.x - 9.0 * z.y + 18.0 * uv.x)));
    float tBound = fract(phase + 0.22 * folds + 0.08 * length(z) + uTime * 0.0001);
    vec3 col = palette(tBound, schemeInt) * (0.60 + 0.40 * folds);
