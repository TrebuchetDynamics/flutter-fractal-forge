...
// Barnsley Julia II: piecewise z=(z±1)·cfix where cfix=(0.6,1.1),
// condition on Re(z·cfix)≥0. Julia-style: z₀=pixel.
// Derivative dz/dz₀: der₀=(1,0), der=cfix·der (same for both branches).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Condition on Re(z·cfix)
    vec2 zc = cmul(z, cfix);
    vec2 shift = (zc.x >= 0.0) ? (z - vec2(1.0, 0.0)) : (z + vec2(1.0, 0.0));
    // d(shift·cfix)/dz₀ = cfix·der (shift differs by constant ±1)
    der = cmul(cfix, der);
    z = cmul(shift, cfix);
