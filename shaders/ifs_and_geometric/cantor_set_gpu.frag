...
void main() {
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    if (digit > 0.5 && digit < 1.5) {
      removed = 1.0;
      depth = float(i + 1);
      break;
    }

    stripe = min(stripe, abs(x - 0.5) / pow(3.0, float(i) * 0.7));
    depth += 1.0;
  }
  ...
  if (removed < 0.5) {
    float t = fract(depth / float(target) + 0.3 * edge + uTime * 0.00008);
  ...
  if (uTransparentBg > 0.5) {
    ...
    float bg = 0.02 + 0.06 * band;
    fragColor = vec4(linearToSRGB(vec3(bg)), 1.0);
  }
}
