...
void main() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    bool centerX = a.x < t;
    bool centerY = a.y < t;
    bool cornerCut = (a.x > 0.32 && a.y > 0.32);

    edgeTrap = min(edgeTrap, min(abs(a.x - t), abs(a.y - t)) / pow(3.0, float(i)));

    if ((centerX && !centerY) || (centerY && !centerX) || cornerCut) {
      removed = 1.0;
  ...
  float t = fract(level + 0.25 * exp(-20.0 * edgeTrap) + uTime * 0.0001);
  vec3 color = palette(t, int(uColorScheme));
  fragColor = vec4(linearToSRGB(color * (0.65 + 0.7 * smoothstep(0.2, 0.0, edgeTrap))), 1.0);
}
