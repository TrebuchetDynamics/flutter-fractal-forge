...
// ── Menger sponge distance estimator (KIFS) ────────────────────────────
// Folds the point into positive octant, sorts axes to bring the largest
// coordinate first, scales by 3, translates, and folds back the Z cross.
// The running scale divisor gives the true world-space distance.
float mengerDE(vec3 p, int maxIter) {
    float scale = max(uPower, 1.5); // default 3.0
    float d = max(max(abs(p.x), abs(p.y)), abs(p.z)) - 1.0; // initial box
    float s = 1.0;

    for (int i = 0; i < 20; i++) {
        if (i >= maxIter) break;

        // Fold: bring into positive octant
        p = abs(p);
...
vec3 getNormal(vec3 pos, int maxIter) {
    float eps = 0.001;
    float d = mengerDE(pos, maxIter);
    return normalize(vec3(
        mengerDE(pos + vec3(eps, 0.0, 0.0), maxIter) - d,
        mengerDE(pos + vec3(0.0, eps, 0.0), maxIter) - d,
        mengerDE(pos + vec3(0.0, 0.0, eps), maxIter) - d
    ));
}
...
float computeAO(vec3 pos, vec3 normal, int maxIter) {
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        ao -= (d - mengerDE(pos + normal * d, maxIter)) * pow(2.0, float(i));
    }
...
vec4 rayMarch(vec3 origin, vec3 direction, int maxIter) {
    ...
    for (int i = 0; i < 150; i++) {
        ...
        float distance = mengerDE(pos, maxIter);
