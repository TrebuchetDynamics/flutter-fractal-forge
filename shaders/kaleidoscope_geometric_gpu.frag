...
// Kaleidoscope Geometric: 3 mirrors in triangle
uniform float uTime;
...
float geometricPattern(vec2 p, float time) {
  vec2 q = fract(p * 3.0 + time * 0.2) - 0.5;
  float tri = min(abs(q.x) + abs(q.y), min(1.0 - abs(q.x) + abs(q.y), abs(q.x) - abs(q.y) + 1.0));
  return clamp(1.0 - smoothstep(0.0, 0.1, tri), 0.0, 1.0);
}
...
void main() {
  ...
  float pat = geometricPattern(kal, uTime);
