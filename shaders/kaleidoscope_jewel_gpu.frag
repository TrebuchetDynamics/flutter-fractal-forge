...
// Kaleidoscope Jewel: 3 mirrors in triangle
uniform float uTime;
...
float jewelPattern(vec2 p, float time) {
  float r = length(p);
  float a = atan(p.y, p.x);
  float facets = abs(sin(a * 8.0)) * exp(-r * 3.0);
  float edge = exp(-abs(r - 0.25 - sin(time) * 0.05) * 30.0);
  return clamp(facets + edge * 0.6, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = jewelPattern(kal, uTime);
