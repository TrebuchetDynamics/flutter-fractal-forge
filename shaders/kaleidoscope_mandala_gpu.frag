...
// Kaleidoscope Mandala: 3 mirrors in triangle
uniform float uTime;
...
float mandalaPattern(vec2 p, float time) {
  float r = length(p);
  float a = atan(p.y, p.x);
  float v = 0.0;
  for (int i = 0; i < 4; i++) {
    float ri = 0.1 + float(i) * 0.12;
    float ring = exp(-abs(r - ri) * 25.0);
    float petals = abs(sin(a * (4.0 + float(i)) + time * (0.5 + float(i) * 0.2)));
    v += ring * (0.5 + petals * 0.5);
  }
...
void main() {
  ...
  float pat = mandalaPattern(kal, uTime);
