...
// Kaleidoscope Nebula: 3 mirrors in triangle
uniform float uTime;
...
float nebulaPattern(vec2 p, float time) {
  vec2 q = p * 2.0 + time * 0.15;
  float n = fbm(q) * 0.6 + fbm(q * 2.0 + time * 0.2) * 0.4;
  float r = length(p);
  n *= exp(-r * 2.0);
...
void main() {
  ...
  float pat = nebulaPattern(kal, uTime);
  
  int scheme = int(mod(uColorScheme, 16.0));
  vec3 col = palette(pat + uTime * 0.12, scheme);
