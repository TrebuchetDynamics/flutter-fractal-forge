...
// Kaleidoscope Rays: 3 mirrors in triangle
uniform float uTime;
...
float raysPattern(vec2 p, float time) {
  float r = length(p);
  float angle = atan(p.y, p.x);
  float spokes = abs(sin(angle * 3.0 + time));
  float ring = exp(-abs(r - 0.3 - sin(time) * 0.1) * 20.0);
  return clamp(spokes * ring + ring * 0.5, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = raysPattern(kal, uTime);
