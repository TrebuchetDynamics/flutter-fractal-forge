...
// Kaleidoscope Ring: 3 mirrors in triangle
uniform float uTime;
...
float ringPattern(vec2 p, float time) {
  ...
  float rings = 0.0;
  for (int i = 0; i < 5; i++) {
    float ri = 0.1 + float(i) * 0.1;
    float wave = sin(time * 2.0 + float(i)) * 0.02;
    rings += exp(-abs(r - ri - wave) * 30.0) * 0.5;
  }
  float spokes = abs(sin(a * 5.0)) * exp(-r * 2.0);
  return clamp(rings + spokes * 0.4, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = ringPattern(kal, uTime);
