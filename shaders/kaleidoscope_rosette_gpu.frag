...
// Kaleidoscope Rosette: 3 mirrors in triangle
uniform float uTime;
...
float rosettePattern(vec2 p, float time) {
  float r = length(p);
  float a = atan(p.y, p.x);
  float petals = cos(a * 6.0 + time) * 0.5 + 0.5;
  petals *= exp(-r * 3.0);
  float center = exp(-r * 8.0);
  return clamp(petals + center * 0.7, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = rosettePattern(kal, uTime);
