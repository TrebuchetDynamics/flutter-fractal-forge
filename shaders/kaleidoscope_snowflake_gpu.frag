...
// Kaleidoscope Snowflake: 3 mirrors in triangle
uniform float uTime;
...
float snowflakePattern(vec2 p, float time) {
  float r = length(p);
  float a = atan(p.y, p.x);
  float branches = abs(sin(a * 6.0)) * exp(-r * 3.0);
  float crystals = sin(p.x * 20.0 + time) * sin(p.y * 20.0 - time) * 0.3;
  return clamp(branches + crystals, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = snowflakePattern(kal, uTime);
