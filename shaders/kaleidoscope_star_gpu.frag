...
// Kaleidoscope Star: 3 mirrors in triangle
uniform float uTime;
...
float starPattern(vec2 p, float time) {
  float r = length(p);
  float a = atan(p.y, p.x);
  float rays = abs(sin(a * 6.0 + time * 0.5));
  float star = rays * exp(-r * 4.0);
  float center = exp(-r * 8.0);
  return clamp(star + center * 0.5, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = starPattern(kal, uTime);
