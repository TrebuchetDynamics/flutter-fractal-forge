...
// Kaleidoscope Crystal: 3 mirrors in triangle
uniform float uTime;
...
float crystalPattern(vec2 p, float time) {
  ...
  float hex = max(abs(p.x) * 0.866 + p.y * 0.5, -p.y);
  hex = 1.0 - smoothstep(0.2, 0.25, hex);
  float lines = abs(sin(a * 6.0)) * exp(-r * 2.0);
  return clamp(hex + lines * 0.5, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = crystalPattern(kal, uTime);
