...
// Kaleidoscope Wave: 3 mirrors in triangle
uniform float uTime;
...
float wavePattern(vec2 p, float time) {
  float wave1 = sin(p.x * 5.0 + time * 1.5) * 0.5 + 0.5;
  float wave2 = sin(p.y * 5.0 - time) * 0.5 + 0.5;
  float r = length(p);
  float rings = sin(r * 8.0 - time * 2.0) * 0.5 + 0.5;
  return clamp(wave1 * wave2 + rings * 0.3, 0.0, 1.0);
}
...
void main() {
  ...
  float pat = wavePattern(kal, uTime);
