...
// KIFS Sierpinski Tetrahedron — 3D ray-marched distance-estimated fractal.
// Kaleidoscopic IFS: at each iteration the point is scaled and folded
// toward the nearest tetrahedron vertex.  uPower controls the scale
// factor (canonical value 2.0, range ~1.5..3.0).
...
// ── Sierpinski tetrahedron distance estimator (KIFS) ───────────────────
// Four tetrahedron vertices in a cube of side 2.  Each iteration folds
// the point toward the closest vertex then scales by uPower.
float sierpinskiTetraDE(vec3 p, int maxIter) {
    float scale = max(uPower, 1.5); // default 2.0

    // Regular tetrahedron vertices (inscribed in unit cube diagonals)
    ...
    for (int i = 0; i < 30; i++) {
        ...
        // Find closest vertex (branchless via min cascade)
        vec3  c    = a1;
        float dist = dot(p - a1, p - a1);

        float d2 = dot(p - a2, p - a2);
        float d3 = dot(p - a3, p - a3);
        float d4 = dot(p - a4, p - a4);

        c = mix(c, a2, step(d2, dist));
        dist = min(dist, d2);
        c = mix(c, a3, step(d3, dist));
        dist = min(dist, d3);
        c = mix(c, a4, step(d4, dist));

        // Scale toward closest vertex
...
vec3 getNormal(vec3 pos, int maxIter) {
    float eps = 0.001;
    float d = sierpinskiTetraDE(pos, maxIter);
    return normalize(vec3(
        sierpinskiTetraDE(pos + vec3(eps, 0.0, 0.0), maxIter) - d,
        sierpinskiTetraDE(pos + vec3(0.0, eps, 0.0), maxIter) - d,
        sierpinskiTetraDE(pos + vec3(0.0, 0.0, eps), maxIter) - d
    ));
}
...
float computeAO(vec3 pos, vec3 normal, int maxIter) {
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        ao -= (d - sierpinskiTetraDE(pos + normal * d, maxIter)) * pow(2.0, float(i));
    }
...
vec4 rayMarch(vec3 origin, vec3 direction, int maxIter) {
    ...
    for (int i = 0; i < 150; i++) {
        ...
        float distance = sierpinskiTetraDE(pos, maxIter);
