...
// KIFS Snowflake Fold — 3D ray-marched distance-estimated fractal.
// Kaleidoscopic IFS with hexagonal (6-fold) symmetry folds producing a
// snowflake-like 3D solid.  Two 60-degree reflections enforce hexagonal
// symmetry, then a bounded Z-fold, scale and translate iterate the
// geometry.  uPower controls the scale factor (default 3.0).
...
uniform float uPower;         // 9   scale factor (default 3.0)
...
// ── Snowflake distance estimator (KIFS, hexagonal folds) ───────────────
// Two reflection vectors at +/-60 degrees create 6-fold symmetry in the
// XY plane.  The Z axis is clamped (bounded fold) to keep the solid
// finite.  The standard IFS scale-translate drives self-similarity.
float snowflakeDE(vec3 p, int maxIter) {
    float scale = max(uPower, 1.5); // default 3.0

    // Unit vectors for 60-degree hexagonal reflections
    // n1 = (-sin30, cos30, 0) = (-0.5, 0.866, 0)
    // n2 = ( sin30, cos30, 0) = ( 0.5, 0.866, 0)
    vec2 n1 = vec2(-0.5, 0.866025);
    vec2 n2 = vec2( 0.5, 0.866025);
...
vec3 getNormal(vec3 pos, int maxIter) {
    float eps = 0.001;
    float d = snowflakeDE(pos, maxIter);
    return normalize(vec3(
        snowflakeDE(pos + vec3(eps, 0.0, 0.0), maxIter) - d,
        snowflakeDE(pos + vec3(0.0, eps, 0.0), maxIter) - d,
        snowflakeDE(pos + vec3(0.0, 0.0, eps), maxIter) - d
    ));
}
...
float computeAO(vec3 pos, vec3 normal, int maxIter) {
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        ao -= (d - snowflakeDE(pos + normal * d, maxIter)) * pow(2.0, float(i));
    }
