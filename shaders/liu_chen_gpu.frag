...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.1 * sin(0.001 * uTime + p.x * 1.3);
  ...
  const float f = 5.58;
  const float g = -1.0;
  const float dt = 0.0018;

  int target = int(clamp(uIterations, 1.0, float(MAX_ITERS)));
  float bailoutSq = max(30.0, uBailout * uBailout);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = a * y + b * x + c * x * z;
    float dy = d * y - z + e * x * z;
    float dz = f * z + g * x * y;
    ...
    orbit += exp(-0.16 * (x * x + y * y)) + 0.08 * exp(-0.65 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
