...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.15 * sin(0.001 * uTime + (p.x - p.y) * 1.8);
  ...
  float abOverApb = (a * b) / max(1e-6, a + b);

  for (int i = 0; i < MAX_ITERS; i++) {
    if (i >= target) break;

    float dx = -abOverApb * x - y * z + c;
    float dy = a * y + x * z;
    float dz = b * z + x * y;
  ...
  if (it >= target) {
    float t = fract((orbit / float(target)) * 2.0 + 0.07 * atan(y, x) + uTime * 0.00005);
