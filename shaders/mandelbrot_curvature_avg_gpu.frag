...
// Mandelbrot Curvature Average Coloring
// Measures orbit curvature at each step: |sin(arg(z_{n+1}-z_n) - arg(z_n-z_{n-1}))|
// The average curvature over the orbit produces smooth banding patterns
// that reveal internal structure orthogonal to standard coloring methods.
...
// sin(arg(b) - arg(a)) = (a.x*b.y - a.y*b.x) / (|a|*|b|)
float curvatureContrib(vec2 d1, vec2 d2) {
  float len1 = length(d1);
  float len2 = length(d2);
  if (len1 < 1e-10 || len2 < 1e-10) return 0.0;
  return abs(d1.x * d2.y - d1.y * d2.x) / (len1 * len2);
}
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    if (j >= 2) {
      curvSum += curvatureContrib(z - zPrev, zPrev - zPrev2);
      curvCount++;
    }
  ...
  float avg = (curvCount > 0) ? curvSum / float(curvCount) : 0.0;
