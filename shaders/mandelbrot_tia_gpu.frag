...
// Mandelbrot Triangle Inequality Average (TIA) Coloring
// Each orbit step contributes (|z_{n+1}| - lo) / (hi - lo) where
//   lo = ||z_n^2| - |c||  and  hi = |z_n^2| + |c|
// The average over all escaped steps drives the color, creating smooth
// organic gradients that reveal structure invisible to escape-time coloring.
// Interior pixels darkened to 40% to distinguish the boundary.
...
void main() {
  ...
  vec2 z  = vec2(0.0);
  vec2 der = vec2(0.0);  // dz/dc for normal-map
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Derivative update (before z)
    der = 2.0 * vec2(z.x*der.x - z.y*der.y, z.x*der.y + z.y*der.x) + vec2(1.0, 0.0);

    // z^2 (before adding c) — used for triangle inequality bounds
    vec2 zSq = vec2(z.x*z.x - z.y*z.y, 2.0*z.x*z.y);
    z = zSq + c;

    // Triangle inequality contribution for this step.
    float zSqLen = length(zSq);
    float lo = abs(zSqLen - cLen);
    float hi = zSqLen + cLen;
    float spread = hi - lo;
    if (spread > 1e-10) {
      tiaSum += clamp((length(z) - lo) / spread, 0.0, 1.0);
      tiaCount++;
    }
  ...
  float avg = (tiaCount > 0) ? tiaSum / float(tiaCount) : 0.0;
  ...
  // Standard: use TIA average as hue, smoothly blended.
  float frac   = fract(smoothVal);
  float tA = fract(avg + uTime * 0.0001);
  float tB = fract(avg + 1.0 / max(1.0, uIterations) + uTime * 0.0001);
  float t  = mix(tA, tB, frac);
