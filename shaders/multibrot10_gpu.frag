...
// Multibrot¹⁰ set: z_{n+1} = z^10 + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// The degree-10 Multibrot set has ten-fold rotational symmetry — a decagonal
// arrangement of ten primary bulbs around the central body. At degree 10 the
// main body is highly compressed but the surrounding escape-time gradient
// reveals extraordinarily rich structure: ten arms each with ten sub-arms,
// creating a dense fractal tree. The degree-10 map also has exact bilateral
// symmetry (even degree) producing a mirror-symmetric parameter space.
// z^10 computed as (z²)⁵ = ((z²)²)·(z²) via squarings for efficiency.
// Smooth coloring: log₂(10). Supports normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z9  = cmul(z8, z);
    vec2 z10 = cmul(z8, z2);
    // Derivative: 10·z⁹·der + 1 (Mandelbrot mode)
    der = 10.0 * cmul(z9, der) + vec2(1.0, 0.0);
    z = z10 + c;
  ...
  float smoothVal = float(it) - log2(log2(mag2)) / log2(10.0);
