...
// Multibrot¹² set: z_{n+1} = z^12 + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// The degree-12 Multibrot set has twelve-fold rotational symmetry — a
// dodecagonal arrangement of twelve primary bulbs. Even degree produces exact
// bilateral mirror symmetry. The twelve-armed star pattern develops exquisite
// self-similar sub-bulbs at every scale. z^12 computed as z⁸·z⁴ via squarings.
// Smooth coloring: log₂(12). Supports normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z11 = cmul(z8, z3);
    vec2 z12 = cmul(z8, z4);
    // Derivative: 12·z¹¹·der + 1 (Mandelbrot mode)
    der = 12.0 * cmul(z11, der) + vec2(1.0, 0.0);
    z = z12 + c;
