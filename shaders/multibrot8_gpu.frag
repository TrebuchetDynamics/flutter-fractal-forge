...
// Multibrot degree 8: z_{n+1} = z^8 + c.
// Generalized Mandelbrot set of the octic power map.
// Degree-8 maps have 7-fold rotational symmetry — the main cardioid
// (actually a disk since 8 is even) is flanked by 7 primary bulbs in a
// heptagon arrangement. The even degree gives a fully symmetric body
// with 7-fold filament fans. At high zoom levels the miniature copies
// of the full set embedded in the filaments also show 7-fold symmetry.
// Derivative: 8z^7·der + 1 for normal-map shading.
// Smooth coloring: it − log₂(log₂(|z|²)) / log₂(8) = it − log₂(log₂(|z|²)) / 3.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z7 = cmul(z6, z);
    vec2 z8 = cmul(z4, z4);
    // Derivative: 8z^7·der + 1
    der = 8.0 * cmul(z7, der) + vec2(1.0, 0.0);
    z = z8 + c;
