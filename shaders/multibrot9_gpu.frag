...
// Multibrot⁹ set: z_{n+1} = z^9 + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// The degree-9 Multibrot set has nine-fold rotational symmetry with nine
// primary bulbs arranged in a nonagon pattern around the main cardioid.
// Higher degree compresses the main body and produces more intricate
// boundary detail: the nine arms each host miniature copies of the full
// set, and the Misiurewicz points create elaborate spiral decorations at
// the antenna tips. Smooth coloring: log₂(9).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z8 = cmul(z4, z4);
    vec2 z9 = cmul(z8, z);
    // Derivative: 9·z⁸·der + 1 (Mandelbrot mode)
    der = 9.0 * cmul(z8, der) + vec2(1.0, 0.0);
    z = z9 + c;
