...
// Multibrot d=2.5: z_{n+1} = z^2.5 + c
// Non-integer power via polar form: z^p = |z|^p · e^(ip·arg(z)).
// At p=2.5 the set has 1.5 "petals" — between the standard Mandelbrot (p=2)
// and the 3-petal multibrot (p=3), with characteristic asymmetric spirals.
// Derivative: d(z^2.5)/dc = 2.5 · z^1.5 · der + 1.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 z15 = cpow(z, 1.5);
    der = 2.5 * cmul(z15, der) + vec2(1.0, 0.0);
  ...
  float smoothVal = float(it) - log2(log2(mag2)) / log2(2.5);
