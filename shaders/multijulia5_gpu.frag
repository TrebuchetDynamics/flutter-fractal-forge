...
// Multijulia degree 5: z_{n+1} = z^5 + c, c = (0.48, 0.18).
// Julia set of the quintic Mandelbrot map — pixel = z₀, c is fixed.
// Degree-5 maps produce 4-fold-symmetric Julia sets with characteristic
// 5-armed star filaments. Near c=(0.48, 0.18) the set has a compact
// connected body with long dendritic spirals at each arm tip.
// Derivative dz/dz₀ = 5z⁴·der for normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  vec2 c   = vec2(0.48, 0.18);   // fixed Julia parameter
