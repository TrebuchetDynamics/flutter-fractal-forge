...
// Multijulia degree 6: z_{n+1} = z^6 + c, c = (0.42, 0.32).
// Julia set of the sextic Mandelbrot map — pixel = z₀, c is fixed.
// The degree-6 map has 5-fold rotational symmetry in its parameter space,
// producing Julia sets with 6-arm star filaments near the origin and
// 5-fold repeating bulb structures along each arm. Near c=(0.42, 0.32)
// the connected set exhibits a central rosette with fine lace detail.
// Derivative dz/dz₀ = 6z⁵·der for normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  vec2 c   = vec2(0.42, 0.32);   // fixed Julia parameter
