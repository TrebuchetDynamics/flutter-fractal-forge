...
// Multijulia degree 7: z_{n+1} = z^7 + c, c = (−0.32, 0.10).
// Julia set of the septimal Mandelbrot map — pixel = z₀, c is fixed.
// Degree-7 maps produce 6-fold-symmetric Julia sets with characteristic
// 7-armed star filaments. Near c=(−0.32, 0.10) the Julia set has a
// connected body with long dendritic spiral arms, each further decorated
// with 6-fold sub-filaments giving a recursive snowflake appearance.
// Derivative dz/dz₀ = 7z^6·der for normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  vec2 c   = vec2(-0.32, 0.10);   // fixed Julia parameter
