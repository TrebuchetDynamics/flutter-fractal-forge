...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.09 * cos(0.001 * uTime + p.x * 1.1 + p.y * 0.9);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = -a * x + y + 10.0 * y * z;
    float dy = -x - 0.4 * y + 5.0 * x * z;
    float dz = b * z - 5.0 * x * y;
    ...
    orbit += exp(-0.22 * (x * x + y * y)) + 0.1 * exp(-0.7 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
  ...
  if (it >= target) {
    float t = fract((orbit / float(target)) * 2.1 + 0.05 * atan(y, x) + uTime * 0.00005);
