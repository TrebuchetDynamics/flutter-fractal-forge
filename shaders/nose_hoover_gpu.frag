...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.1 * sin(0.001 * uTime + p.x - p.y);

  const float dt = 0.008;

  int target = int(clamp(uIterations, 1.0, float(MAX_ITERS)));
  float bailoutSq = max(12.0, uBailout * uBailout);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    orbit += exp(-0.22 * (x * x + y * y)) + 0.09 * exp(-0.8 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
  ...
  if (it >= target) {
    float t = fract((orbit / float(target)) * 2.2 + 0.05 * atan(y, x) + uTime * 0.00005);
