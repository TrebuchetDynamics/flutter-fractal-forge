...
// Nova Degree-4 (Mandelbrot mode): z_{n+1} = (3z⁴+1)/(4z³) + c, z₀ = 1.
// The Nova map applies one step of Newton's method for z⁴−1=0 and adds c.
// In Mandelbrot mode: pixel = c, z₀ = 1+0i for all c. The resulting
// parameter space features a large central attractor basin surrounded by
// intricate filamentary structures where the orbit diverges. The quartic
// Nova map has four-fold symmetry from the underlying roots ±1, ±i.
...
void main() {
  ...
  vec2 der = vec2(0.0);         // dz/dc starts at 0
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Mandelbrot derivative: der_new = N'(z)·der + 1
