...
void main() {
  ...
  // Pickover cross trap: minimum of |Re(z)| and |Im(z)| across orbit
  float trapMin = 1e10;
  int trapIter = 0;
  ...
  if (!inMainBulb(c)) {
    ...
    for (int j = 0; j < MAX_ITERS; j++) {
      ...
      // Cross trap: distance to the real and imaginary axes
      float crossDist = min(abs(z.x), abs(z.y));
      if (crossDist < trapMin) {
        trapMin = crossDist;
        trapIter = j;
      }
  ...
  if (it >= target) {
    // Inside set: still color by trap
    if (trapMin < 1e9) {
  ...
  // Color by trap distance and iteration
