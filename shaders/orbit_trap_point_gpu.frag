...
uniform float uTrapReal;
uniform float uTrapImag;
...
void main() {
  ...
  vec2 trapPoint = vec2(uTrapReal, uTrapImag);
  ...
  // Point orbit trap: minimum |z - p| across orbit
  float trapMin = 1e10;
  int trapIter = 0;
  ...
  if (!inMainBulb(c)) {
    ...
    for (int j = 0; j < MAX_ITERS; j++) {
      ...
      // Point trap: distance from orbit point to trap point
      float pointDist = length(z - trapPoint);
      if (pointDist < trapMin) {
        trapMin = pointDist;
        trapIter = j;
      }
  ...
  float brightness = 1.0 - clamp(trapMin * 1.5, 0.0, 0.8);
