...
// Perpendicular Burning Ship Cubic set: z_{n+1} = (|Re(z)| + i·Im(z))^3 + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// The Perpendicular Burning Ship cubic folds only the real part of z through
// absolute value before raising to the third power, unlike the standard Burning
// Ship which folds both components. At degree 3 the fold creates an asymmetric
// three-armed parameter space with the right arm displaying the smooth arcs of
// the standard cubic and the left pair of arms showing the doubled-lobe structure
// from the Re-fold. The bulbs along the positive-real axis retain Mandelbrot-like
// cardioid character while the folded half shows crescent cusps.
// Derivative approximation: 3·(|Re(z)|+i·Im(z))²·der + 1 (Mandelbrot mode).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 w2  = cmul(w, w);
    vec2 w3  = cmul(w2, w);
    // Derivative approximation: 3w²·der + 1 (Mandelbrot mode)
    der = 3.0 * cmul(w2, der) + vec2(1.0, 0.0);
    z = w3 + c;
