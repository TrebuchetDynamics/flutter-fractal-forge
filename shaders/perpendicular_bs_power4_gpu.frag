...
// Perpendicular Burning Ship⁴: z_{n+1} = (Re z + i|Im z|)⁴ + c.
// Mandelbrot analogue — pixel = c, z₀ = 0.
// Only the imaginary component is reflected: za = (Re z, |Im z|).
// This one-sided fold is perpendicular to the standard Burning Ship fold
// (which folds both components). The degree-4 map produces a four-armed
// structure where only the lower half-plane is folded, creating asymmetric
// ship-like features offset from those of the standard Burning Ship⁴.
// Smooth coloring: log₂(4) = 2. Supports normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Perpendicular fold: only imaginary component
    vec2 za  = vec2(z.x, abs(z.y));
    vec2 za2 = cmul(za, za);
    vec2 za3 = cmul(za2, za);
    vec2 za4 = cmul(za2, za2);
    // Derivative: 4·za³·der + 1 (Mandelbrot mode; fold approximated)
    der = 4.0 * cmul(za3, der) + vec2(1.0, 0.0);
    z = za4 + c;
