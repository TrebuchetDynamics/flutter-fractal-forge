...
// Perpendicular Burning Ship: z_{n+1} = (|Re(z)| + i·Im(z))^2 + c.
// Variant of the Burning Ship where only the real part is folded through
// absolute value before squaring, leaving the imaginary part unchanged.
// Expanding: (|x|+iy)^2 = x^2-y^2 + i·2|x|y — the imaginary part is
// 2|Re(z)|·Im(z), giving Im always the same sign as Im(z). This produces
// a distinctive "ship" body oriented perpendicular to the classic Burning
// Ship, with flame-like filaments extending along the imaginary axis and
// the main cardioid slightly tilted relative to Mandelbrot.
// Derivative approximation: 2(|Re|+iIm)·der + 1 for normal map.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Fold Re only: w = (|Re(z)|, Im(z))
    vec2 w = vec2(abs(z.x), z.y);
    // Derivative: 2w·der + 1 (Re fold approximation)
    der = 2.0 * cmul(w, der) + vec2(1.0, 0.0);
    // z_{n+1} = w^2 + c = (x^2-y^2, 2|x|y) + c
