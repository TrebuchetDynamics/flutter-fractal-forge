...
// Perpendicular Burning Ship Julia set: z_{n+1} = (|Re(z)| + i·Im(z))^2 + c,
// c = (−0.90, 0.30). Julia set of the Perpendicular Burning Ship map —
// pixel = z₀, c = (−0.90, 0.30) is fixed.
// The Perpendicular Burning Ship folds only the real part through absolute value
// before squaring, unlike the standard Burning Ship which folds both components.
// This produces asymmetric Julia sets with ship-like cusp features on one side
// and smooth Mandelbrot-like boundaries on the other — a hybrid character unique
// to this perpendicular variant. Near c=(−0.90, 0.30) the set is connected with
// a large central body and asymmetric filaments extending rightward.
// Derivative: 2·(|Re(z)|+i·Im(z))·der — Julia mode (no +1 term).
...
void main() {
  ...
  vec2 c   = vec2(-0.90, 0.30);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Perpendicular fold: |Re(z)| only, Im(z) unchanged
    vec2 w = vec2(abs(z.x), z.y);
    // Derivative: 2w·der (Julia mode — no +1 term)
    der = 2.0 * cmul(w, der);
