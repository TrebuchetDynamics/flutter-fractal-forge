...
// Perpendicular Mandelbrot (Celtic): z_{n+1} = |Re(z^2)| + i·Im(z^2) + c
// Abs applied to the real part of z^2 only (opposite of Buffalo).
...
void main() {
  ...
  // z_{n+1} = |Re(z^2)| + i·Im(z^2) + c
  // d(Re(z^2))/dc = Re(2z·der); abs gate: sign(Re(z^2)) * d(Re(z^2))/dc
  // d(Im(z^2))/dc = Im(2z·der)
  vec2 der = vec2(0.0);
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float x2 = z.x * z.x - z.y * z.y;  // Re(z^2)
    float y2 = 2.0 * z.x * z.y;         // Im(z^2)

    // Derivative update (before z update)
    float dx2 = 2.0 * (z.x * der.x - z.y * der.y);  // Re(2z·der)
    float dy2 = 2.0 * (z.x * der.y + z.y * der.x);  // Im(2z·der)
