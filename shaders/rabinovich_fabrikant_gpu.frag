...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.08 * sin(0.001 * uTime + p.x * 1.5 - p.y * 1.1);

  const float alpha = 0.14;
  const float gamma = 0.10;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = y * (z - 1.0 + x * x) + gamma * x;
    float dy = x * (3.0 * z + 1.0 - x * x) + gamma * y;
    float dz = -2.0 * z * (alpha + x * y);
    ...
    orbit += exp(-0.20 * (x * x + y * y)) + 0.10 * exp(-0.7 * abs(z));
    if (r2 > bailoutSq) {
      it = i + 1;
      break;
    }
