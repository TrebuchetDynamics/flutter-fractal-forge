...
// Robinson attractor — α=0.4, β=0.8, γ=0.5, δ=1.0.
// dx/dt = y
// dy/dt = x(1 − x²) − αy + βz
// dz/dt = xy − γz
// A Duffing-type double-well oscillator extended to 3D by the z-feedback
// and xy-coupling. The x(1−x²) restoring force creates two equilibria at
// x=±1; the z coupling drives switching between wells producing the
// characteristic double-scroll with fine laminar structure.
uniform float uTime;
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.1 * sin(0.0015 * uTime) + 0.05 * p.x;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    if (i >= target) break;
    float dx = y;
    float dy = x*(1.0 - x*x) - alpha*y + beta*z;
    float dz = x*y - gamma*z;
    x += dt * dx;
    y += dt * dy;
    z += dt * dz;
    float r2 = x*x + y*y + 0.3*z*z;
    orbit += exp(-0.30 * (x*x + y*y)) + 0.07 * exp(-0.6 * z*z);
