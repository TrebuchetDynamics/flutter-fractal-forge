...
// Chebyshev-Halley parametric family on z^3 - 1.
// L = f*f'' / (f')^2
// z_new = z - (1 + L/(2*(1 - lambda*L))) * f/f'
// Extra param: lambda (slot 10, default 0.0)
// lambda=0 -> Chebyshev, lambda=0.5 -> Halley, lambda=1 -> Super-Halley
...
uniform float uLambda;        // 10 (default 0.0)
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Correction: 1 + L / (2*(1 - lambda*L))
    vec2 lambdaL = lambda * Lf;
    vec2 denom = 2.0 * (vec2(1.0, 0.0) - lambdaL);
    vec2 corr = vec2(1.0, 0.0) + cdiv(Lf, denom);

    vec2 step = cmul(corr, newtonStep);
    z = z - step;
