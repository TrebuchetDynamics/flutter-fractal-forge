...
// Noor-Newton three-step iteration on z^3 - 1.
// N(z) = z - f(z)/f'(z)  (Newton map)
// x = (1-a)*z + a*N(z)
// y = (1-b)*z + b*N(x)
// z_new = (1-c)*z + c*N(y)
// Extra params: a (slot 10), b (slot 11), c (slot 12)
...
uniform float uParamA;        // 10 (default 0.5)
uniform float uParamB;        // 11 (default 0.5)
uniform float uParamC;        // 12 (default 0.5)
...
// Newton map: N(w) = w - f(w)/f'(w) for f(w) = w^3 - 1
vec2 newtonMap(vec2 w) {
  vec2 w2 = cmul(w, w);
  vec2 w3 = cmul(w2, w);
  vec2 fw = w3 - vec2(1.0, 0.0);
  vec2 fpw = 3.0 * w2;
  return w - cdiv(fw, fpw);
}
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // x = (1-a)*z + a*N(z)
    vec2 nz = newtonMap(z);
    vec2 x = (1.0 - pa) * z + pa * nz;

    // y = (1-b)*z + b*N(x)
    vec2 nx = newtonMap(x);
    vec2 y = (1.0 - pb) * z + pb * nx;
