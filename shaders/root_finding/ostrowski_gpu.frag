...
// Ostrowski's method on z^3 - 1.
// Two-step: y = z - f(z)/f'(z)
//           z_new = y - f(y)/f'(z) * f(z)/(f(z) - 2*f(y))
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Ostrowski correction: z_new = y - f(y)/f'(z) * f(z)/(f(z) - 2*f(y))
    vec2 ratio1 = cdiv(fy, fpz);
    vec2 ratio2 = cdiv(fz, fz - 2.0 * fy);
    vec2 step = z - y + cmul(ratio1, ratio2);

    vec2 zNew = y - cmul(ratio1, ratio2);
