...
// Steffensen's method on z^3 - 1.
// z_{n+1} = z - f(z)^2 / (f(z + f(z)) - f(z))
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 fz = f_z3m1(z);
    vec2 w = z + fz;           // w = z + f(z)
    vec2 fw = f_z3m1(w);       // f(w) = f(z + f(z))
    vec2 denom = fw - fz;      // f(w) - f(z)
    vec2 fz2 = cmul(fz, fz);   // f(z)^2
    vec2 step = cdiv(fz2, denom);
