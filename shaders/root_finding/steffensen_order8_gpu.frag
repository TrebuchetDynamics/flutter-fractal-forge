...
// Steffensen's method order-8 on z^3 - 1.
// Triple composition of Steffensen steps for 8th-order convergence.
// Each Steffensen step: w = z + f(z); z_next = z - f(z)^2 / (f(w) - f(z))
// Apply three consecutive steps per iteration.
...
// Single Steffensen step
vec2 steffensenStep(vec2 z) {
  vec2 fz = f_z3m1(z);
  vec2 w = z + fz;
  vec2 fw = f_z3m1(w);
  vec2 denom = fw - fz;
  vec2 fz2 = cmul(fz, fz);
  return z - cdiv(fz2, denom);
}
