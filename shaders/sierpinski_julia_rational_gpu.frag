...
// Sierpinski-Julia Rational Map: z → z² + c/z²
// Julia sets homeomorphic to Sierpinski gaskets via Misiurewicz-type rational maps.
// The rational singularity at z=0 creates carpet/gasket-like structures at
// critical c values, producing fractal boundaries with Sierpinski topology.
// Derivative (Julia-style dz/dz₀):
//   f(z) = z² + c/z²
//   f'(z) = 2z − 2c/z³
//   derNew = f'(z) · der, der₀ = (1,0)
...
uniform float uCReal;         // 10  (default -0.1, range -2..2)
uniform float uCImag;         // 11  (default 0.0, range -2..2)
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float zMag2 = dot(z, z);
    // Guard against z near zero (rational singularity)
    if (zMag2 < 1e-20) { it = j; break; }

    vec2 z2 = cmul(z, z);
    vec2 z3 = cmul(z2, z);

    // f'(z) = 2z - 2c/z^3
    vec2 fpz = 2.0 * z - 2.0 * cdiv(c, z3);
    der = cmul(fpz, der);

    // f(z) = z^2 + c/z^2
    vec2 cOverZ2 = cdiv(c, z2);
    z = z2 + cOverZ2;
