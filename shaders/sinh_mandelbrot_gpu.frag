...
// Sinh Mandelbrot: z_{n+1} = sinh(z) + c
// Transcendental escape-time fractal. sinh produces period-2 bulb structures
// mirrored about the real axis; boundary has dendritic Cantor-like arms.
// Derivative: d(sinh(z))/dc = cosh(z)·der + 1.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    if (j >= target) break;
    // d(sinh(z))/dc = cosh(z)·der + 1
    der = cmul(ccosh(z), der) + vec2(1.0, 0.0);
    z   = csinh(z) + c;
