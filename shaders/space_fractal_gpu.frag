...
// Space fractal — from MandlebrotSetSFML formula #19.
// z_{n+1} = z² + c / (z + 0.01).
// The 0.01 offset prevents the singularity at z=0 while creating deep
// spiral arms reminiscent of galaxy structures in the parameter plane.
// With the right palette and zoom level the Julia sets resemble outer space.
// Derivative: der_{n+1} = (2z − c/(z+0.01)²)·der + 1/(z+0.01).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 zOff  = z + vec2(0.01, 0.0);      // z + 0.01  (offset denominator)
    vec2 zOff2 = cmul(zOff, zOff);         // (z+0.01)²
    // der_{n+1} = (2z − c/(z+0.01)²)·der + 1/(z+0.01)
    der = cmul(2.0*z - cdiv(c, zOff2), der) + cdiv(vec2(1.0, 0.0), zOff);
    z   = cmul(z, z) + cdiv(c, zOff);      // z² + c/(z+0.01)
