...
// Sprott E — from Sprott (1994) "Some simple chaotic flows", case E.
// dx/dt = yz,  dy/dt = x² − y,  dz/dt = 1 − 4x.
// The linear z-equation (1−4x) creates a folded sheet geometry; the
// attractor has a distinctive figure-eight cross-section in the xy-plane.
uniform float uTime;
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.4 + 0.2 * sin(0.0017 * uTime) + 0.1 * p.y;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    orbit += exp(-0.20 * (x*x + y*y)) + 0.08 * exp(-0.5 * abs(z));
