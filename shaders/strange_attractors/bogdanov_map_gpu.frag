...
// Bogdanov map — y' = y + eps*y + k*x*(x-1) + mu*x*y, x' = x + y'.
...
uniform float uParamEps;  // slot 10, default 0.1
uniform float uParamK;    // slot 11, default 1.2
uniform float uParamMu;   // slot 12, default 0.5
...
void main() {
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float ny = y + eps * y + k * x * (x - 1.0) + mu * x * y;
    float nx = x + ny;
    x = nx;
    y = ny;
  ...
  if (it >= target) {
    float t = fract((density / float(TRANSIENT)) * 1.5 + uTime * 0.00005);
