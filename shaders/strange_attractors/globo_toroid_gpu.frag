...
// Globo-Toroid attractor — toroidal winding chaotic system.
// dx/dt = −y + ax·z
// dy/dt = x + by·z
// dz/dt = c − x² − y²
// A minimal 3D system with toroidal invariant structure. Without the xz/yz
// coupling the x-y plane rotates uniformly. The quadratic z-restoring term
// c−x²−y² drives z toward the torus surface r²=c; the small axz/byz
// perturbations break integrability and produce the chaotic winding orbit.
// Parameters: a=0.5, b=0.5, c=1.0.
uniform float uTime;
...
void main() {
  ...
  float z = 0.05 * sin(0.0014 * uTime) + 0.03 * (p.x + p.y);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = -y + a_p*x*z;
    float dy = x + b_p*y*z;
    float dz = c_p - x*x - y*y;
    x += dt * dx;
    y += dt * dy;
    z += dt * dz;
    float r2 = x*x + y*y + z*z;
    orbit += exp(-0.25 * (x*x + y*y)) + 0.08 * exp(-2.0 * z*z);
