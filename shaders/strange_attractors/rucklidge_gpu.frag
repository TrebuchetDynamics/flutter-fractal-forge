...
// Rucklidge attractor (1992) — κ=2, λ=6.7.
// dx/dt = −κx + λy − yz  = −2x + 6.7y − yz
// dy/dt = x
// dz/dt = −z + y²
// Generates a pair of intersecting spiral lobes. Originally derived as a
// model for magnetoconvection in an idealized fluid layer. The κ term
// provides damping while λ drives the instability; yz creates the nonlinear
// coupling that produces chaos.
uniform float uTime;
...
void main() {
  ...
  // z typically lives around 4–8; seed near the attractor
  float z = 4.5 + 0.5 * sin(0.0011 * uTime) + 0.2 * (p.x + p.y);

  const float kappa  = 2.0;
  const float lambda = 6.7;
  const float dt = 0.01;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    orbit += exp(-0.08 * (x*x + y*y)) + 0.05 * exp(-0.1 * abs(z - 5.0));
