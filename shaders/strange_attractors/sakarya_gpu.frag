...
// Sakarya attractor — a=0.4, b=0.3.
// dx/dt = −x + y + yz
// dy/dt = −x − y + axz
// dz/dt = z + bxz − y²
// A relatively simple 3D chaotic system with quadratic cross terms. The
// bxz product in dz and yz in dx provide the nonlinear coupling needed for
// chaos. Produces a compact single-lobe attractor near the origin.
uniform float uTime;
...
void main() {
  ...
  float z = 0.1 * sin(0.0012 * uTime) + 0.05 * (p.x - p.y);
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    if (i >= target) break;
    float dx = -x + y + y*z;
    float dy = -x - y + a_p*x*z;
    float dz = z + b_p*x*z - y*y;
    x += dt * dx;
    y += dt * dy;
    z += dt * dz;
    float r2 = x*x + y*y + 0.2*z*z;
    orbit += exp(-0.25 * (x*x + y*y)) + 0.07 * exp(-0.4 * z*z);
