...
void main() {
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float nx = alpha * (y - x - gamma * x * x * x);
    float ny = x - y + z;
    float nz = -beta * y - delta * z + 0.65 * x;
    ...
    float px = x + 0.35 * z;
    float py = y - 0.22 * z;
    float r2 = px * px + py * py + 0.10 * z * z;
    density += exp(-0.22 * (px * px + py * py)) + 0.12 * exp(-0.9 * abs(z));
  ...
  if (it >= target) {
    float ang = atan(y + 0.3 * z, x - 0.2 * z);
    float t = fract((density / float(target)) * 2.1 + 0.06 * ang + uTime * 0.00005);
