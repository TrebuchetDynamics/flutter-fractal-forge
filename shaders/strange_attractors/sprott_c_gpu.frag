...
// Sprott C — from Sprott (1994) "Some simple chaotic flows", case C.
// dx/dt = yz,  dy/dt = x − y,  dz/dt = 1 − x².
// Closely related to Sprott B; the z-equation uses x² instead of xy,
// producing a different topology of basins and attractor cross-sections.
uniform float uTime;
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.5 + 0.2 * sin(0.0013 * uTime) + 0.1 * p.y;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    trap = min(trap, abs(z) + 0.25 * abs(x - y));
    orbit += exp(-0.20 * (x*x + y*y)) + 0.08 * exp(-0.6 * abs(z));
    if (r2 > bailoutSq) { it = i + 1; break; }
  }

  if (it >= target) {
    float strands = exp(-5.0 * trap);
    float t = fract((orbit / float(target)) * 2.4 + 0.18 * strands + 0.08 * atan(y, x) + uTime * 0.00005);
    vec3 color = getPaletteColor(t, int(uColorScheme));
    color = mix(color * 0.72, color + vec3(0.25, 0.18, 0.06), smoothstep(0.12, 0.85, strands));
