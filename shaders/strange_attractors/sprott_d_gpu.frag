...
// Sprott D — from Sprott (1994) "Some simple chaotic flows", case D.
// dx/dt = −y,  dy/dt = x + z,  dz/dt = xz + 3y².
// The 3y² term creates large excursions in z; the attractor has a
// distinctive pretzel-like cross-section in the xz-plane.
uniform float uTime;
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.1 * sin(0.0009 * uTime) + 0.1 * p.x;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    float dx = -y;
    float dy = x + z;
    float dz = x*z + 3.0*y*y;
    x += dt * dx;
    y += dt * dy;
    z += dt * dz;
    float r2 = x*x + y*y + 0.1*z*z;
    orbit += exp(-0.25 * (x*x + y*y)) + 0.05 * exp(-0.4 * abs(z));
