...
// Sprott F — from Sprott (1994) "Some simple chaotic flows", case F.
// dx/dt = y + z,  dy/dt = −x + y/2,  dz/dt = x² − z.
// The y/2 damping term and x²-z restoring force interact to produce a
// single-scroll attractor with a distinctive C-shaped cross-section.
uniform float uTime;
...
void main() {
  ...
  float x = p.x;
  float y = p.y;
  float z = 0.3 * sin(0.0015 * uTime) + 0.2 * p.x;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    ...
    orbit += exp(-0.22 * (x*x + y*y)) + 0.08 * exp(-0.5 * abs(z));
