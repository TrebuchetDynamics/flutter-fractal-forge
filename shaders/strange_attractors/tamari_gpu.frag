...
// Tamari attractor — a=50, b=0.833, c=20.
// dx/dt = a(y − x)
// dy/dt = (b − a)x + bx·z
// dz/dt = xy − cz
// A Lorenz-family system with non-standard y-coupling. The bxz product in dy
// (rather than the Lorenz −xz) shifts the sign of the stretching term,
// producing spiral scrolls that coil in the opposite direction to Lorenz.
// The large a value creates strong contraction; the small b/c ratio generates
// a compact multi-layered double-scroll with fine internal structure.
uniform float uTime;
...
void main() {
  ...
  const float PHASE_SCALE = 12.0;
  float x = p.x * PHASE_SCALE;
  float y = p.y * PHASE_SCALE;
  float z = 0.5 + 0.2 * sin(0.0010 * uTime) + 0.05 * length(p);

  const float a_p = 50.0;
  const float b_p =  0.833;
  const float c_p = 20.0;
  ...
  for (int i = 0; i < MAX_ITERS; i++) {
    if (i >= target) break;
    float dx = a_p*(y - x);
    float dy = (b_p - a_p)*x + b_p*x*z;
    float dz = x*y - c_p*z;
    x += dt * dx;
    y += dt * dy;
    z += dt * dz;
    float r2 = x*x + y*y + 0.05*z*z;
    orbit += exp(-0.08 * (x*x + y*y)) + 0.05 * exp(-0.1 * abs(z - 0.5));
