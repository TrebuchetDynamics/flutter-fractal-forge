...
// Tangent Julia set: f(z) = c · tan(z), c = (0.12, 0.48).
// The complex tangent has poles at z = π/2 + kπ, creating "traps" that
// produce intricate canal structures. The Julia set alternates between
// Cantor-bouquet filaments and disk-shaped Fatou components.
// Derivative: d/dz [c·tan(z)] = c·sec²(z) = c / cos²(z).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Complex sin and cos
    vec2 cz_sin = vec2(sin(z.x)*cosh(z.y),  cos(z.x)*sinh(z.y));
