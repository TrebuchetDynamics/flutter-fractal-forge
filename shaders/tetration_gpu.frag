...
// Tetration Fractal: z_{n+1} = c^z  (c=pixel, z₀=(1,0))
// Iterated exponentiation (power tower): c^c^c^… diverges or converges
// depending on c. Produces ornate, self-similar antenna-like structures.
// Derivative (dz/dc): der_{n+1} = z_{n+1}·(log(c)·der_n + z_n/c), der₀=(0,0).
...
vec2 clogSafe(vec2 z) {
  float r2 = max(dot(z, z), 1e-20);
  return vec2(0.5 * log(r2), atan(z.y, z.x));
}

vec2 cexpSafe(vec2 z) {
  float ex = exp(clamp(z.x, -80.0, 80.0));
  return ex * vec2(cos(z.y), sin(z.y));
}

vec2 cpowSafe(vec2 a, vec2 b) {
  return cexpSafe(cmul(b, clogSafe(a)));
}
...
void main() {
  ...
  // Chain rule: d(c^z)/dc = c^z·(log(c)·der + z/c)
  vec2 der = vec2(0.0);
  vec2 logC = clogSafe(c);
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 zNext = cpowSafe(c, z);
    // d(c^z)/dc = c^z · (log(c)·der + z/c)
    der = cmul(zNext, cmul(logC, der) + cdivSafe(z, c));
    z   = zNext;
