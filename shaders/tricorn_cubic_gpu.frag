...
// Tricorn Cubic (Mandelbar³): z_{n+1} = conj(z)^3 + c
// conj(z)^3 = (Re(z^3), -Im(z^3)) — the conjugate applied before cubing.
// Combines the anti-holomorphic symmetry of Tricorn with 3-fold branching,
// producing asymmetric 3-armed filament structures.
...
void main() {
  ...
  vec2 z   = vec2(0.0);
  // Derivative dz/dc for normal-map. Tricorn Cubic: z = conj(z)^3 + c.
  // conj(z)^3 = (Re(z^3), -Im(z^3))
  // d(conj(z)^3)/dc = (Re(3*z^2*der), -Im(3*z^2*der))
  // = (3*(z2r*der.x - z2i*der.y), -(3*(z2r*der.y + z2i*der.x)))
  vec2 der = vec2(0.0);
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    float x2 = z.x*z.x;
    float y2 = z.y*z.y;
    float x3 = z.x*(x2 - 3.0*y2);  // Re(z^3)
    float y3 = z.y*(3.0*x2 - y2);  // Im(z^3)

    // Derivative update: d(conj(z^3))/dc = (Re(3z^2·der), -Im(3z^2·der))
    float z2r = x2 - y2;
    float z2i = 2.0*z.x*z.y;
    float dx3 = 3.0*(z2r*der.x - z2i*der.y);
    float dy3 = 3.0*(z2r*der.y + z2i*der.x);
    der = vec2(dx3 + 1.0, -dy3);

    // Tricorn Cubic: z = (Re(z^3), -Im(z^3)) + c = conj(z^3) + c
    z = vec2(x3 + c.x, -y3 + c.y);
