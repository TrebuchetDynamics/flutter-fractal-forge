...
// Tricorn (Mandelbar) degree 4: z_{n+1} = conj(z)^4 + c.
// The conjugation before squaring breaks holomorphicity, producing
// a set with 4-fold anti-holomorphic symmetry. The boundary is not
// smooth — it has cusp-like features at every period-doubling bifurcation.
// Unlike the quadratic Tricorn (z̄²+c), degree-4 produces 3 primary
// "antennas" per arm. Supports normal-map shading (colorScheme 50-63).
uniform float uTime;
...
void main() {
  ...
  // Early-out: z0=0 => z1=c for conjugate-power Mandelbrot variants too.
  if (dot(c, c) > bailoutSq) {
    ...
    for (int j = 0; j < MAX_ITERS; j++) {
      ...
      // Conjugate z, then raise to power 4
      vec2 zc  = vec2(z.x, -z.y);   // conj(z)
      vec2 zc2 = cmul(zc, zc);       // conj(z)^2
      vec2 zc4 = cmul(zc2, zc2);     // conj(z)^4
      // Derivative for normal map (anti-holomorphic: d/dc̄ is used, approximated)
      vec2 zc3 = cmul(zc2, zc);
      der = 4.0 * cmul(zc3, vec2(der.x, -der.y)) + vec2(1.0, 0.0);
      z = zc4 + c;
