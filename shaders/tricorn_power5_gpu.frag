...
// Tricorn degree 5 (Mandelbar ⁵): z_{n+1} = conj(z)^5 + c.
// Extends the classic Tricorn (conj(z)^2+c) to degree 5.
// The anti-holomorphic map (conjugation before power) breaks
// the Cauchy–Riemann equations and produces cusp-like bulbs
// rather than the smooth cardioids of the Mandelbrot family.
// Five-fold anti-rotational symmetry gives star-shaped bulb clusters.
// Derivative: 5·conj(z)⁴·conj(der) for normal-map shading.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Conjugate z, then raise to power 5
    vec2 zc   = vec2(z.x, -z.y);      // conj(z)
    vec2 derc = vec2(der.x, -der.y);  // conj(der)
    vec2 zc2 = cmul(zc, zc);
    vec2 zc4 = cmul(zc2, zc2);
    // Derivative: 5·conj(z)⁴·conj(der)
    der = 5.0 * cmul(zc4, derc) + vec2(1.0, 0.0);
    z = cmul(zc4, zc) + c;
