...
// Tricorn⁵ Julia set: z_{n+1} = conj(z)^5 + c, c = (0.28, 0.40).
// Julia set of the degree-5 Tricorn (Mandelbar) map — pixel = z₀, c = (0.28, 0.40) fixed.
// Conjugating z before raising to the fifth power creates an anti-holomorphic map
// with five-fold symmetry (period-5 structure in Fatou components). The Julia set
// shows five primary arms with needle-like cusps at arm tips — characteristic of
// odd-degree anti-holomorphic maps. Near c=(0.28, 0.40) the set is connected
// with radial symmetry visible across five sectors.
// Derivative: 5·conj(z)⁴·conj(der) — anti-holomorphic, Julia mode (no +1 term).
...
void main() {
  ...
  vec2 c   = vec2(0.28, 0.40);   // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Anti-holomorphic derivative: 5·zc⁴·conj(der), Julia mode (no +1)
    der = 5.0 * cmul(zc4, derc);
    z = cmul(zc4, zc) + c;
