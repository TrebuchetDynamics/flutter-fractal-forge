...
// Tricorn⁶ Julia set: z_{n+1} = conj(z)^6 + c, c = (0.08, 0.45).
// Julia set of the degree-6 Tricorn (Mandelbar) map — pixel = z₀, c = (0.08, 0.45) fixed.
// Conjugating z before raising to the sixth power creates an anti-holomorphic map
// with six-fold symmetry in its parameter space. The Julia set shows six primary
// arms with the distinctive needle-cusp boundaries of anti-holomorphic maps.
// At degree 6, the Fatou components form pairs of symmetric bulbs along each arm,
// giving a rosette appearance with fine secondary filaments at the arm boundaries.
// Near c=(0.08, 0.45) the set is connected with visible hexagonal symmetry.
// Derivative: 6·conj(z)⁵·conj(der) — anti-holomorphic, Julia mode (no +1 term).
...
void main() {
  ...
  vec2 c   = vec2(0.08, 0.45);    // fixed Julia parameter
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Anti-holomorphic derivative: 6·conj(z)⁵·conj(der), Julia mode (no +1)
    der = 6.0 * cmul(zc5, derc);
    z = zc6 + c;
