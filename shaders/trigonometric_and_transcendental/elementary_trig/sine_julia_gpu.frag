...
// Sine Julia: z_{n+1} = c · sin(z), c = (−0.70176, −0.3842)
// Julia set of the transcendental map f(z) = c·sin(z). Produces elongated
// bands and spiral arms similar to the cosine Julia but rotated 90°.
// Derivative (Julia-style dz/dz₀): dz/dz₀ = cmul(c, cos(z))·der, der₀ = 1.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // d(c·sin(z))/dz₀ = c·cos(z)·der
    der = cmul(cmul(c, ccos(z)), der);
    z   = cmul(c, csin(z));
  ...
  if (it >= target) {
    ...
    float bands = smoothstep(0.36, 0.50, abs(sin(10.0 * z.x + 7.0 * z.y + 16.0 * uv.x)));
