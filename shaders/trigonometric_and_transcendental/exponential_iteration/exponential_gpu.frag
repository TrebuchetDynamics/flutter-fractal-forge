...
// Exponential Mandelbrot: z_{n+1} = exp(z) + c
// The entire complex plane maps periodically (period 2πi), producing an infinite
// cascade of Mandelbrot-like bulbs along the imaginary axis with dendritic arms.
// Derivative: d(exp(z))/dc = exp(z)·der + 1.
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // d(exp(z))/dc = exp(z)·der + 1; reuse ez for both z update and derivative.
    vec2 ez = cexpSafe(z);
    der = cmul(ez, der) + vec2(1.0, 0.0);
    z   = ez + c;
