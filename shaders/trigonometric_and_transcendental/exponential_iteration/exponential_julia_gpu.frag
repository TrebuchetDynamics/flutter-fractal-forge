...
// Exponential Julia set: z_{n+1} = e^z + c, c = (−0.65, 0.0).
// Julia set of the complex exponential map f_c(z) = e^z + c.
// Unlike polynomial maps, the exponential has an essential singularity
// at infinity, so the Julia set is the boundary between bounded and
// escaping orbits with no simple degree. Near c=(−0.65, 0.0) the
// connected Julia set exhibits Cantor bouquets: infinite-length curves
// with a Cantor-set structure at each cross-section, producing
// intricate hair-like filaments radiating from a central body.
// Escape: |z| > bailout (use large bailout ~50).
...
vec2 cexp(vec2 z) {
  float ex = exp(clamp(z.x, -88.0, 88.0));
...
void main() {
  ...
  vec2 c   = vec2(-0.65, 0.0);   // fixed Julia parameter
  ...
  // Smooth coloring approximation for transcendental escape
