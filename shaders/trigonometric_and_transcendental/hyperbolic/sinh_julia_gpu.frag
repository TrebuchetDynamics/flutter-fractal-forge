...
// Sinh Julia set: f(z) = c · sinh(z), c = (−0.65, 0.45).
// The hyperbolic sine is entire (no poles), creating Julia sets with
// long spiral arms and intricate period-doubling structures mirroring
// the sine Julia but with exponential growth instead of oscillation.
// Derivative: d/dz [c·sinh(z)] = c·cosh(z).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    // Derivative: d[c·sinh(z)]/dz₀ = c·cosh(z)·der
    der = cmul(cmul(c, cz_cosh), der);
    z   = cmul(c, cz_sinh);
  ...
  if (it >= target) {
    ...
    float arms = smoothstep(0.26, 0.52, abs(sin(10.0 * (uv.x + uCenter.x) + 14.0 * sinh(clamp(uv.y + uCenter.y, -1.4, 1.4)))));
    float bands = smoothstep(0.30, 0.56, abs(sin(16.0 * (uv.y + uCenter.y) + 8.0 * phase)));
    float tBound = fract(phase + 0.22 * arms + 0.12 * bands + uTime * 0.0001);
    vec3 col = palette(tBound, schemeInt) * (0.48 + 0.52 * max(arms, bands));
