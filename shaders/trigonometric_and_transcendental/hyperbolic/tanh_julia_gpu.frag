...
// Tanh Julia set: f(z) = c · tanh(z), c = (0.80, 0.40).
// Hyperbolic tangent maps ℂ to the horizontal strip |Im|<π/2 with
// poles at i(π/2 + kπ). The Julia set alternates between quasi-circles
// (resembling quadratic Julia sets in the bounded regime) and
// intricate fractal canals near the pole lines.
// Derivative: d/dz [c·tanh(z)] = c / cosh²(z).
...
void main() {
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 cz_tanh = cdiv(cz_sinh, cz_cosh);
    // sech²(z) = 1 / cosh²(z)
    vec2 cz_cosh2 = cmul(cz_cosh, cz_cosh);
    vec2 cz_sech2 = cdiv(vec2(1.0, 0.0), cz_cosh2);
    // Derivative: d[c·tanh(z)]/dz₀ = c·sech²(z)·der
    der = cmul(cmul(c, cz_sech2), der);
    z   = cmul(c, cz_tanh);
  ...
  if (it >= target) {
    ...
    float disks = smoothstep(0.26, 0.52, abs(sin(18.0 * length(uv + uCenter) + 10.0 * phase)));
    float canals = smoothstep(0.30, 0.56, abs(sin(16.0 * (uv.y + uCenter.y) + 6.0 * tanh(uv.x + uCenter.x))));
    float tBound = fract(phase + 0.22 * disks + 0.12 * canals + uTime * 0.0001);
    vec3 col = palette(tBound, schemeInt) * (0.48 + 0.52 * max(disks, canals));
