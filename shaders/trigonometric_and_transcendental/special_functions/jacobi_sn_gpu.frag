...
// Jacobi sn Fractal: z_{n+1} = sn(z,k) + c
// sn(z,k) ~ sin(z) - k^2*(sin(z) - z*cos(z))*cos^2(z)/6
// First-order perturbation approximation of Jacobi elliptic sn function.
// Extra param: k (elliptic modulus) at slot 10 (default 0.5)
...
void main() {
  ...
  // ponytail: Jacobi approximation uses complex sin/cos each step; cap for catalog rendering.
  ...
  for (int j = 0; j < MAX_ITERS; j++) {
    ...
    vec2 sz = cx_sin(z);
    vec2 cz = cx_cos(z);
    vec2 cz2 = cx_mul(cz, cz);
    vec2 correction = cx_mul(sz - cx_mul(z, cz), cz2);
    vec2 sn = sz - k2 / 6.0 * correction;
    z = sn + c;
