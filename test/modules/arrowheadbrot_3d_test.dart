...
void main() {
  ...
  test('Arrowheadbrot is one fixed quadratic principal tricomplex slice', () {
    final module = ModuleRegistry().byId('arrowheadbrot_3d');
  ...
  test('shader locks the published real-axis idempotent components', () {
    final shader = File(asset).readAsStringSync();

    expect(shader, contains('vec2 cMinus = vec2(point.x - point.z, point.y);'));
    expect(shader, contains('vec2 cPlus = vec2(point.x + point.z, point.y);'));
    expect(shader, contains('float complexMandelbrotDistance(vec2 c)'));
    expect(shader, contains('sqrt(0.5 * (minusDistance * minusDistance +'));
  });

  test('known points distinguish Arrowheadbrot from Tetrabrot', () {
    final result = RenderMathOracle.evaluate('arrowheadbrot_3d');
