...
void main() {
  ...
  test('Rule 150 shader uses trinomial parity, not neighboring Rule 90 rows',
    ...
    expect(
        shader, contains('for (int bitIndex = 0; bitIndex < 11; bitIndex++)'));
    expect(shader, isNot(contains('combo < 6561')));
    expect(shader,
        isNot(contains('rule90State(gen, cell - 1) + rule90State(gen, cell)')));
  });
}
