...
@test "pre-release gate dry run includes every mandatory host and device gate" {
  ...
  [[ "$output" == *"integration_test/performance/catalog_thumbnail_performance_test.dart"* ]]
  [[ "$output" == *"FORCE_RUNTIME_CATALOG_THUMBNAILS=true"* ]]
  [[ "$output" == *"FORCE_GPU_RENDER=true"* ]]
  ...
  [[ "$output" == *"guard foreground package com.trebuchetdynamics.fractal.forge"* ]]
  [ "$(grep -c "guard foreground package com.trebuchetdynamics.fractal.forge" <<<"$output")" -eq 5 ]
  [[ "$output" == *"guard foreground package com.trebuchetdynamics.fractal.forge while soak runs"* ]]
